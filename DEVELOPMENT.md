...
- [Joining the FluxGit Team](#joining-the-fluxgit-team)
...
For a deep dive into the architecture, see [DEEPWIKI](https://deepwiki.com/fluxgitapp/fluxgit).
...
First, run cargo build such that supplementary bins such as `fluxgit-git-askpass` are created (Note the default folder (`target`) will be used for the build):
...
```shell
GITBUTLER_PERFORMANCE_LOG=1 /Applications/FluxGit/Contents/MacOS/fluxgit-tauri
```

###### For the nightly build

```shell
GITBUTLER_PERFORMANCE_LOG=1 /Applications/FluxGit\ Nightly.app/Contents/MacOS/fluxgit-tauri
```
...
This will pop open an SVG version of the graph that FluxGit uses to create the workspace projection.
...
Common issues and solutions when developing FluxGit.
...
If none of these solutions work, please check our [GitHub Issues](https://github.com/fluxgitapp/fluxgit/issues) or create a new issue with detailed information about your system and the error you're encountering.
...
```bash
$ pnpm tauri build --features devtools,builtin-but,disable-auto-updates --config crates/fluxgit-tauri/tauri.conf.nightly-local.json
```
...
The `windows` dependency on `fluxgit-git` doesn't currently compile on ARM,
...
```
pnpm tauri build --features windows,devtools --config  crates/fluxgit-tauri/tauri.conf.nightly.json
```
...
design of FluxGit, or your work involves UI, you could duplicate our design file.

FluxGit design: [Figma file](https://www.figma.com/file/FbeLt0yjY9RiNn8MXUXsYs/Client-Design?type=design&node-id=0%3A1&mode=design&t=MUDQhR3iOM3DpI9m-1) 🎨
...
Most of this is for internal FluxGit use, but maybe everyone else will find
it interesting too.
...
Building is done via [GitHub Action](https://github.com/fluxgitapp/fluxgit/actions/workflows/publish.yaml).
Go to the link and select `Run workflow` from the desired branch.

### Versioning

When running the [release action](https://github.com/fluxgitapp/fluxgit/actions/workflows/publish.yaml),
...
version found at `https://app.fluxgit.com/releases`.

### Publishing

To publish a version that you've just build, use [Release Manager](https://fluxgit.retool.com/apps/cb9cbed6-ae0a-11ed-918c-736c4335d3af/Release%20Manager).

---

## Development mode OAuth login

By default, you will not be able to log into FluxGit using Github/Google because the base url does not match. To be able to do this add ( or update ) the following line to your `.env.development` file. You will need to create the file if it does not exist.

```
PUBLIC_API_BASE_URL=https://app.fluxgit.com/
```

---

## Joining the FluxGit Team
...
- [Senior Rust developer](https://jobs.fluxgit.com/jobs/backend-rust) (SF, Berlin or Remote)
- [Senior TypeScript developer](https://jobs.fluxgit.com/jobs/frontend-typescript) (SF, Berlin or Remote)
- [Gerrit developer](https://jobs.fluxgit.com/jobs/gerrit-developer) (SF, Berlin or Remote)

## Code Hitlist

This is a list of crates/modules that we want to eliminate or split into smaller crates:

- [fluxgit-reference](crates/fluxgit-reference/) (just bad)
- [fluxgit-branch-actions](crates/fluxgit-branch-actions/) (contains functionality outside of the virtual branch domain (e.g. commit actions etc.))
- [fluxgit-branch](crates/fluxgit-branch/) (contains `diff` and `branch` contexts due to a cyclic dependency)
- [fluxgit-url](crates/fluxgit-url/) (this is a huge mess and ideally we need none of it)
- [fluxgit_repo::config](crates/fluxgit-repo/src/config.rs) (seems like the wrong abstraction)
