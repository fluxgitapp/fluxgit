...
This directory contains all end-to-end (E2E) tests for FluxGit. We use two different testing frameworks to cover different scenarios:
...
```typescript
import { startFluxGit, type FluxGit } from "../src/setup.ts";
...
test("should do something amazing", async ({ page, context }, testInfo) => {
	...
	// Setup test data
	await fluxgit.runScript("your-setup-script.sh");
...
3. **Use test utilities** from `playwright/src/`:
   - `setup.ts`: For starting FluxGit instances
...
describe("Your Feature", () => {
	before(() => {
		...
		spawnAndLog("bash", [
			"-c",
			"./blackbox/scripts/your-setup-script.sh ../target/debug/fluxgit-cli",
		]);
	});
...
│   │   ├── setup.ts        # FluxGit instance setup
