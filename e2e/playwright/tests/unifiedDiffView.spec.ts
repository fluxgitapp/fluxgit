...
test("should be able to select the hunks correctly in a complex file", async ({
	...
	const projectPath = fluxgit.pathInWorkdir(projectName + "/");
	...
	await fluxgit.runScript("project-with-remote-branches__commit-file-into-remote-base.sh", [
		"Create big file with complex changes",
		fileName,
		contentBefore,
	]);
	// Clone into a new project
	await fluxgit.runScript("project-with-remote-branches__clone-into-new-project.sh", [
		projectName,
	]);
	// Delete the other project to avoid having to switch between them
	await fluxgit.runScript("project-with-remote-branches__delete-project.sh", ["local-clone"]);
