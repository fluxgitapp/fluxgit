import { getBaseURL, getButlerPort, type FluxGit, startFluxGit } from "../src/setup.ts";
...
test("should show commit-msg hook rejection error", async ({ page, context }, testInfo) => {
	...
	fluxgit = await startFluxGit(workdir, configdir, context, { RUST_LOG: "debug" });

	await fluxgit.runScript("project-with-commit-hooks.sh");
...
test("should reject commit when pre-commit hook fails", async ({ page, context }, testInfo) => {
	...
	await fluxgit.runScript("create-forbidden-file.sh");
...
test("should allow commit when pre-commit hook passes", async ({ page, context }, testInfo) => {
	...
	await fluxgit.runScript("create-allowed-file.sh");
...
test("should show post-commit hook failure but commit still created", async ({
	...
	await fluxgit.runScript("create-postcommit-fail-marker.sh");
