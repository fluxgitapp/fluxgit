...
test("should be able to apply a remote branch", async ({ page, context }, testInfo) => {
	...
	// const projectPath = fluxgit.pathInWorkdir('local-clone/');
...
test("should be able to apply a remote branch and integrate the remote changes - simple", async ({
	...
	await fluxgit.runScript("project-with-remote-branches__add-commit-to-remote-branch.sh");
...
test("should be able to apply a remote branch and integrate the remote changes - create commit", async ({
	...
	const fileCPath = fluxgit.pathInWorkdir("local-clone/c_file");

	await fluxgit.runScript("project-with-remote-branches.sh");
	await fluxgit.runScript("apply-upstream-branch.sh", ["branch1", "local-clone"]);
...
test("should be able to apply a remote branch and integrate the remote changes - conflict", async ({
	...
	const filePath = fluxgit.pathInWorkdir("local-clone/a_file");
...
test("should be able gracefully handle adding a branch that is ahead of our target commit", async ({
	...
	const fileBPath = fluxgit.pathInWorkdir("local-clone/b_file");
	...
	await fluxgit.runScript("project-with-remote-branches__add-commit-to-base-and-branch.sh");
...
test("should be able gracefully handle adding a branch that is behind of our target commit", async ({
	...
	// There are remote changes in the base branch
	await fluxgit.runScript("project-with-remote-branches__add-commit-to-base.sh");
...
test("should update the stale selection of an unexisting branch", async ({
	...
	await fluxgit.runScript("merge-upstream-branch-to-base.sh", ["branch1"]);
...
test("should be able to delete a local branch", async ({ page, context }, testInfo) => {
	...
	await fluxgit.runScript("project-with-remote-branches.sh");
	await fluxgit.runScript("apply-upstream-branch.sh", ["branch1", "local-clone"]);
	await fluxgit.runScript("apply-upstream-branch.sh", ["branch3", "local-clone"]);
	await fluxgit.runScript("move-branch.sh", ["branch3", "branch1", "local-clone"]);
