...
class FluxGitManager implements FluxGit {
...
function getButlerDataDir(configDir: string): string {
	return path.join(configDir, "com.fluxgit.app");
}
...
export async function startFluxGit(
	workdir: string,
	configDir: string,
	context: BrowserContext,
	env?: Record<string, string>,
	...
	const manager = new FluxGitManager(workdir, configDir, env, config);
	await manager.init();
