#!/bin/sh
# FluxGit installer bootstrap script
# This script downloads and runs the FluxGit installer binary
...
INSTALLER_JSON=$(mktemp "${TMPDIR:-/tmp}/fluxgit-installer-json.XXXXXX")
INSTALLER_BIN=$(mktemp "${TMPDIR:-/tmp}/fluxgit-installer-bin.XXXXXX")
trap 'rm -f "$INSTALLER_JSON" "$INSTALLER_BIN"' EXIT INT TERM

# Fetch installer metadata and validate effective URL
INSTALLER_API_URL="https://app.fluxgit.com/installers/info/$INSTALLER_OS/$INSTALLER_ARCH"
...
case "$INSTALLER_URL" in
    https://releases.fluxgit.com/*)
        : # Valid
        ;;
    *)
        echo "Error: Installer URL is not from a trusted FluxGit domain: $INSTALLER_URL" >&2
        exit 1
        ;;
esac
