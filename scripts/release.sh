...
CONFIG_PATH=$(readlink -f "$PWD/../crates/fluxgit-tauri/tauri.conf.$CHANNEL.json")

if [ "$OS" = "windows" ]; then
  # WARNING: `builtin-but` doesn't work on Windows, see https://github.com/fluxgitapp/fluxgit/issues/11461.
  ...
  EXTERNAL_BIN='["fluxgit-git-askpass", "fluxgit-git-setsid", "but"]'
	FEATURES="windows"
else
  # we need to include the fluxgit-git-setsid dummy binary as
  ...
  EXTERNAL_BIN='["fluxgit-git-askpass", "fluxgit-git-setsid"]'
