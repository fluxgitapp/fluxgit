...
It contains different rust packages, with `fluxgit-tauri` for the tauri application,
...
# look at https://github.com/fluxgitapp/fluxgit/blob/fd9a58de5579074c0526193143d531c21907a26b/scripts/install-tauri-debian-dependencies.sh
...
pnpm tauri build --features devtools --config crates/fluxgit-tauri/tauri.conf.nightly.json
```
...
- No relative imports (use `@fluxgit/` package references)
...
pnpm add <package> --filter @fluxgit/desktop
pnpm add <package> --filter @fluxgit/ui
```
...
- `fluxgit-*` prefix: Core FluxGit functionality. These are old, and we want to port them to `but-*`.
...
- Use absolute imports via package references (e.g., `@fluxgit/ui`) instead of relative imports
