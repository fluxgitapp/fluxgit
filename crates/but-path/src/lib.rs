...
pub fn app_data_dir() -> anyhow::Result<PathBuf> {
    if let Some(test_dir) = std::env::var_os("E2E_TEST_APP_DATA_DIR") {
        return Ok(PathBuf::from(test_dir).join("com.fluxgit.app"));
    }
    dirs::data_dir()
...
pub fn app_config_dir() -> anyhow::Result<PathBuf> {
    if let Some(test_dir) = std::env::var_os("E2E_TEST_APP_DATA_DIR") {
        return Ok(PathBuf::from(test_dir).join("fluxgit"));
    }
    dirs::config_dir()
        .ok_or(anyhow::anyhow!("Could not get app data dir"))
        .map(|dir| dir.join("fluxgit"))
}

/// Returns the platform-specific cache directory for FluxGit, **one per channel**.
///
...
/// # Platform-specific locations
///
/// - **macOS**: `~/Library/Caches/com.fluxgit.app{channel}/`
/// - **Linux**: `~/.cache/com.fluxgit.app{channel}/` (following XDG Base Directory Specification)
/// - **Windows**: `%LOCALAPPDATA%\com.fluxgit.app{channel}\`
///
/// # Testing
///
...
impl AppChannel {
    pub fn new() -> Self {
        match identifier() {
            "com.fluxgit.app.nightly" => AppChannel::Nightly,
            "com.fluxgit.app" => AppChannel::Release,
            _ => AppChannel::Dev,
        }
    }

    /// Open the FluxGit GUI application for `possibly_project_dir`.
    ///
    ...
    pub fn open(&self, possibly_project_dir: &std::path::Path) -> anyhow::Result<()> {
        ...
        // Read more https://github.com/fluxgitapp/fluxgit/pull/11234#issuecomment-3533202481
