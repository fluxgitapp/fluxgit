// use fluxgit_user::{credentials, credentials::count as count_secrets};
use std::path::{Path, PathBuf};

use fluxgit_user::User;
...
fn auto_migration_of_secrets_on_when_getting_and_setting_user() -> anyhow::Result<()> {
    for (name, has_github_token) in [("login-only.v1", false), ("with-github.v1", true)] {
        ...
        assert!(
            fluxgit_user::get_user_with_path(app_data.path())?.is_none(),
        ...
        let user = fluxgit_user::get_user_with_path(app_data.path())?
        ...
        let user =
            fluxgit_user::get_user_with_path(app_data.path())?.expect("stored user can be read");
        assert_access_token_values(&user)?;

        fluxgit_user::delete_user_with_path(app_data.path())?;
        assert_eq!(
            count_secrets(),
            0,
        ...
        fluxgit_user::set_user_with_path(app_data.path(), &user)?;
        assert_eq!(
            count_secrets(),
            expected_secrets,
        ...
        assert!(
            fluxgit_user::get_user_with_path(app_data.path())?.is_some(),
