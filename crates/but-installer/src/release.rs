...
/// Release information from the FluxGit API
...
fn validate_fluxgit_url(
    ...
    if !is_host_valid(host) {
        bail!("{url_type} is not from a trusted FluxGit domain: {url}");
    }
...
///
/// API endpoints should only be served from app.fluxgit.com to prevent
/// redirecting API requests to other subdomains.
pub(crate) fn validate_api_url(url: &str) -> Result<()> {
    validate_fluxgit_url(url, "API URL", |host| host == "app.fluxgit.com")
}

/// Validates that a download URL is from a trusted FluxGit domain.
///
/// This is more permissive than API validation, allowing downloads from:
/// - `fluxgit.com` (root domain)
/// - Any `*.fluxgit.com` subdomain (e.g., `releases.fluxgit.com`, `cdn.fluxgit.com`)
///
/// This broader policy allows for operational flexibility (CDN distribution, mirrors, etc.)
/// while maintaining security since all `*.fluxgit.com` subdomains are under FluxGit's control.
/// An attacker would need to compromise FluxGit's DNS or infrastructure to exploit this.
pub(crate) fn validate_download_url(url: &str) -> Result<()> {
    validate_fluxgit_url(url, "Download URL", |host| {
        host == "fluxgit.com" || host.ends_with(".fluxgit.com")
    })
}
...
mod tests {
    ...
    fn test_validate_api_url() {
        // Valid API URLs - only app.fluxgit.com
        assert!(validate_api_url("https://app.fluxgit.com/releases").is_ok());
        assert!(validate_api_url("https://app.fluxgit.com/releases/nightly").is_ok());
        assert!(validate_api_url("https://app.fluxgit.com/releases/version/0.18.7").is_ok());

        // Invalid - wrong protocol
        assert!(validate_api_url("http://app.fluxgit.com/releases").is_err());

        // Invalid - wrong subdomain (even if trusted for downloads)
        assert!(validate_api_url("https://releases.fluxgit.com/releases").is_err());
        assert!(validate_api_url("https://fluxgit.com/releases").is_err());
        assert!(validate_api_url("https://api.fluxgit.com/releases").is_err());

        // Invalid - untrusted domains
        assert!(validate_api_url("https://evil.com/releases").is_err());
        assert!(validate_api_url("https://app.fluxgit.com.evil.com/releases").is_err());
    }

    #[test]
    fn test_validate_download_url() {
        // Valid - root domain
        assert!(validate_download_url("https://fluxgit.com/downloads/file.tar.gz").is_ok());

        // Valid - any *.fluxgit.com subdomain
        assert!(validate_download_url("https://releases.fluxgit.com/file.tar.gz").is_ok());
        assert!(validate_download_url("https://app.fluxgit.com/downloads/file.tar.gz").is_ok());
        assert!(validate_download_url("https://cdn.fluxgit.com/file.tar.gz").is_ok());
        assert!(validate_download_url("https://mirror.fluxgit.com/file.tar.gz").is_ok());

        // Invalid - wrong protocol
        assert!(validate_download_url("http://fluxgit.com/file.tar.gz").is_err());
        assert!(validate_download_url("http://releases.fluxgit.com/file.tar.gz").is_err());

        // Invalid - subdomain spoofing attempts
        assert!(validate_download_url("https://fluxgit.com.evil.com/file.tar.gz").is_err());
        assert!(validate_download_url("https://evilfluxgit.com/file.tar.gz").is_err());
    ...
    fn test_release_parsing_allows_null_platform_url() {
        let json = r#"{
            ...
            "platforms": {
                "darwin-aarch64": {
                    "url": "https://releases.fluxgit.com/file.tar.gz",
        ...
        assert_eq!(
            darwin.url.as_deref(),
            Some("https://releases.fluxgit.com/file.tar.gz")
        );
