...
pub fn download_and_install_app(
    ...
    info(&format!("Downloading FluxGit {}...", release.version));
...
pub(crate) fn install_app(app_dir: &Path, home_dir: &Path, channel: Option<Channel>) -> Result<()> {
    ...
    if but_symlink.exists() && !but_symlink.is_symlink() {
        ...
        } else if existing_target_str.contains("/FluxGit.app/") {
        ...
        if previous_channel.is_none() {
            ...
            warn("This will be replaced with FluxGit's 'but' command");
            info(
                "Note: Your custom symlink setup will be overwritten. The original target is not a FluxGit installation.",
            );
        }
    }

    // Create temporary symlink to test the new installation
    let new_app_macos_dir = install_app_new.join("Contents/MacOS/fluxgit-tauri");
    ...
    let final_target = install_app.join("Contents/MacOS/fluxgit-tauri");
    ...
    if let Err(e) = symlink_result {
        ...
        if install_app_backup.exists() {
            ...
            let restored_target = install_app.join("Contents/MacOS/fluxgit-tauri");
            let _ = fs::remove_file(&but_symlink);
    ...
    success("FluxGit CLI (but) installed successfully");
...
pub(crate) fn verify_app_structure(app_dir: &Path) -> Result<()> {
    ...
    let required_binaries = ["fluxgit-git-askpass", "fluxgit-tauri"];
