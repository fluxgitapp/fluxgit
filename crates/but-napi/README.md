...
`but-napi` is used to build FluxGit's Node native add-on via `napi-rs`.
...
- Provide the loadable `.node` binary for `@fluxgit/but-sdk`.
...
2. Build bindings with `pnpm --filter @fluxgit/but-sdk build:napi`.
