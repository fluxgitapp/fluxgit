...
mod spawn {
    ...
    use fluxgit_filemonitor::{InternalEvent, WatchMode};
    ...
    async fn track_directory_changes_after_rename() -> anyhow::Result<()> {
        ...
        let monitor = fluxgit_filemonitor::spawn(project_id, &workdir, tx, WatchMode::Modern)?;
...
mod watch_mode {
    use fluxgit_filemonitor::WatchMode;
