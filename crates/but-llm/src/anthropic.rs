...
const ANTHROPIC_KEY_OPTION: &str = "fluxgit.aiAnthropicKeyOption";
const ANTHROPIC_MODEL_NAME: &str = "fluxgit.aiAnthropicModelName";
pub const GB_ANTHROPIC_API_BASE: &str = "https://app.fluxgit.com/api/proxy/anthropic";
...
impl AnthropicClient {
    ...
    fn api_base(&self) -> &str {
        match self.kind {
            CredentialsKind::FluxGitProxied => GB_ANTHROPIC_API_BASE,
...
impl AnthropicProvider {
    pub fn with(preferred_creds: Option<CredentialsKind>, model: Option<String>) -> Option<Self> {
        let credentials = if let Some(kind) = preferred_creds {
            match kind {
                ...
                CredentialsKind::FluxGitProxied => AnthropicProvider::fluxgit_proxied_creds(),
            }
        } else {
            AnthropicProvider::fluxgit_proxied_creds()
    ...
    fn fluxgit_proxied_creds() -> Result<(CredentialsKind, Sensitive<String>)> {
        let creds = secret::retrieve("fluxgit_access_token", secret::Namespace::BuildKind)?
            .ok_or(anyhow::anyhow!(
                "No FluxGit token available. Log-in to use the FluxGit Anthropic provider"
            ))?;
    ...
    fn anthropic_own_key_creds() -> Result<(CredentialsKind, Sensitive<String>)> {
        let creds = secret::retrieve("aiAnthropicKey", secret::Namespace::Global)?.ok_or(
            anyhow::anyhow!(
                "No Anthropic own key configured. Add this through the FluxGit settings"
            ),
        )?;
