...
pub const GB_OPENAI_API_BASE: &str = "https://app.fluxgit.com/api/proxy/openai";

const OPEN_AI_KEY_OPTION: &str = "fluxgit.aiOpenAIKeyOption";
const OPEN_AI_MODEL_NAME: &str = "fluxgit.aiOpenAIModelName";
const OPEN_AI_CUSTOM_ENDPOINT: &str = "fluxgit.aiOpenAICustomEndpoint";
...
impl CredentialsKind {
    fn from_git_config(config: &gix::config::File<'static>) -> Option<Self> {
        ...
        match key_option {
            ...
            CredentialsKeyOption::ButlerApi => Some(CredentialsKind::FluxGitProxied),
        }
    }
}
...
impl OpenAiProvider {
    pub fn with(
        ...
        let credentials = if let Some(kind) = preferred_creds {
            match kind {
                ...
                CredentialsKind::FluxGitProxied => OpenAiProvider::fluxgit_proxied_creds(),
            }
        } else {
            OpenAiProvider::fluxgit_proxied_creds()
    ...
    fn fluxgit_proxied_creds() -> Result<(CredentialsKind, Sensitive<String>)> {
        let creds = secret::retrieve("fluxgit_access_token", secret::Namespace::BuildKind)?
            .ok_or(anyhow::anyhow!(
                "No FluxGit token available. Log-in to use the FluxGit OpenAI provider"
            ))?;
        Ok((CredentialsKind::FluxGitProxied, creds))
    }
    ...
    fn openai_own_key_creds() -> Result<(CredentialsKind, Sensitive<String>)> {
        let creds =
            secret::retrieve("aiOpenAIKey", secret::Namespace::Global)?.ok_or(anyhow::anyhow!(
                "No OpenAI own key configured. Add this through the FluxGit settings"
            ))?;
...
impl OpenAIClientProvider for OpenAiProvider {
    fn client(&self) -> Result<Client<OpenAIConfig>> {
        match &self.credentials {
            ...
            (CredentialsKind::FluxGitProxied, key) => {
