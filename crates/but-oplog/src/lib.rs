...
//! ### Snapshots
//!
//! For simplicity, snapshots of "what we are interested in" is all state that contributes to the FluxGit experience, such as:
//!
...
//! themselves and voluntarily via stashes. We only manipulate data that is stored in Git or that is owned and maintained by FluxGit.
//!
...
//! Note that the `fluxgit-oplog` is still the backbone of this implementation, but modified to the extent necessary.
...
pub struct UnmaterializedOplogSnapshot {
    ...
    #[cfg(feature = "legacy")]
    details: fluxgit_oplog::entry::SnapshotDetails,
}

/// legacy types for easy of use, all provided by `fluxgit-oplog`.
#[cfg(feature = "legacy")]
pub mod legacy {
    pub use fluxgit_oplog::entry::{OperationKind, SnapshotDetails, Trailer};
}
