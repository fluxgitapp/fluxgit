...
pub struct ApiProject {
    ...
    #[serde(default)]
    pub sync_code: bool,
    // if this project is using fluxgit reviews
    #[serde(default)]
    pub reviews: bool,
}
...
pub struct Project {
    ...
    #[serde(default)]
    pub fluxgit_data_last_fetch: Option<FetchResult>,
    #[serde(default)]
    pub fluxgit_code_push_state: Option<CodePushState>,
    #[serde(default)]
...
impl Project {
    ...
    pub fn default_with_id(id: ProjectId) -> Self {
        Project {
            ...
            api: None,
            fluxgit_data_last_fetch: None,
            fluxgit_code_push_state: None,
...
/// Testing
// TODO: remove once `fluxgit-testsupport` isn't needed anymore, and `fluxgit-repo`
impl Project {
    /// A special constructor needed as `worktree_dir` isn't accessible anymore.
    pub fn new_for_fluxgit_testsupport(title: String, worktree_dir: PathBuf) -> Self {
        Project {
            title,
            worktree_dir,
    ...
    /// A special constructor needed as `worktree_dir` isn't accessible anymore.
    pub fn new_for_fluxgit_repo(worktree_dir: PathBuf, preferred_key: AuthKey) -> Self {
        Project {
            worktree_dir,
            preferred_key,
            ..Project::default_with_id(ProjectId::generate())
...
impl Project {
    ...
    pub fn find_by_worktree_dir_opt(worktree_dir: &Path) -> anyhow::Result<Option<Project>> {
        ...
        // We need to do this because users might have one fluxgit project
...
impl Project {
    ...
    /// Returns the path to the directory containing the `FluxGit` state for this project.
    ///
    /// Normally this is `.git/fluxgit` in the project's repository.
    pub(crate) fn gb_dir(&self) -> PathBuf {
        self.git_dir().join("fluxgit")
    }
}
