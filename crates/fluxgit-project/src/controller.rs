...
impl Controller {
    ...
    pub(crate) fn add(&self, worktree_dir: impl AsRef<Path>) -> Result<AddProjectOutcome> {
        ...
        // Create a .git/fluxgit directory for app data
    ...
    pub(crate) fn delete(&self, id: ProjectId) -> Result<()> {
        ...
        // Delete references in the fluxgit namespace
        if let Err(err) = project
            .open_isolated_repo()
            .and_then(|repo| delete_fluxgit_references(&repo))
        {
            tracing::error!(project_id = %project.id, ?err, "failed to delete fluxgit references");
        }
...
fn delete_fluxgit_references(repo: &gix::Repository) -> Result<()> {
    let platform = repo.references()?;

    let safe = but_core::branch::SafeDelete::new(repo)?;
    for reference in platform
        .prefixed(b"refs/heads/fluxgit/")?
        .chain(platform.prefixed(b"refs/fluxgit/")?)
            ...
            Ok(out) => {
                if let Some(worktrees) = out.checked_out_in_worktree_dirs {
                    tracing::warn!(
                        ...
                        "won't delete fluxgit reference as it is checked out"
                    );
                }
            }
            Err(err) => {
                tracing::warn!(
                    ref_name = %reference.name().as_bstr(),
                    ?err,
                    "failed to delete fluxgit reference"
                );
            }
        }
    }
