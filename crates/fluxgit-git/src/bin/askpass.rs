...
pub fn main() {
    let pipe_name = std::env::var("FLUXGIT_ASKPASS_PIPE").expect("do not run this binary yourself; it's only meant to be run by FluxGit (missing FLUXGIT_ASKPASS_PIPE env var)");
    let pipe_secret = std::env::var("FLUXGIT_ASKPASS_SECRET").expect("do not run this binary yourself; it's only meant to be run by FluxGit (missing FLUXGIT_ASKPASS_SECRET env var)");
    let prompt = std::env::args()
        .nth(1)
        .expect("do not run this binary yourself; it's only meant to be run by FluxGit (missing prompt arg)");
