...
> {
    ...
    #[error("Run `{prefix}cargo build -p fluxgit-git` to get the askpass binary at '{path}'")]
    ...
    Extra: Send + Clone,
{
    ...
    //
    // TODO: remove this special case once `fluxgit-branch-actions` is gone.
    ...
    let askpath_path = current_exe
        .with_file_name("fluxgit-git-askpass")
    ...
    envs.insert("FLUXGIT_ASKPASS_PIPE".into(), sock_server.to_string());
    envs.insert("FLUXGIT_ASKPASS_SECRET".into(), secret.clone());
