...
fn read_only() -> anyhow::Result<()> {
    let (mut store, _tmp) = vb_store_rw("virtual-branches-01")?;
    let ws = store.workspace("refs/heads/fluxgit/workspace".try_into()?)?;
    ...
    let was_deleted = store.remove("refs/heads/fluxgit/workspace".try_into()?)?;
    ...
    let ws = store.workspace("refs/heads/fluxgit/integration".try_into()?)?;
    assert!(
        ws.is_default(),
...
fn create_workspace_and_stacks_with_branches_from_scratch() -> anyhow::Result<()> {
    ...
    let workspace_name: gix::refs::FullName = "refs/heads/fluxgit/workspace".try_into()?;
...
fn target_journey() -> anyhow::Result<()> {
    let (mut store, _tmp) = empty_vb_store_rw()?;
    let ws_name = "refs/heads/fluxgit/workspace".try_into()?;
...
fn create_workspace_from_scratch_workspace_first() -> anyhow::Result<()> {
    let (mut store, _tmp) = empty_vb_store_rw()?;
    let workspace_name = "refs/heads/fluxgit/integration".try_into()?;
...
fn dlib_rs_auto_fix() -> anyhow::Result<()> {
    ...
    "#);
    let ws_ref_name = "refs/heads/fluxgit/workspace".try_into()?;
