...
use git2::Tree;
use fluxgit_reference::{Refname, RemoteRefname};
...
///
/// For now, it collects useful methods from `fluxgit-core::git::Repository`
pub trait RepositoryExt {
    ...
    /// Returns the `fluxgit/workspace` branch if the head currently points to it, or fail otherwise.
    /// Use it before any modification to the repository, or extra defensively each time the
    /// workspace is needed.
    ///
    /// This is for safety to assure the repository actually is in 'fluxgit mode'.
...
impl RepositoryExt for git2::Repository {
    ...
    fn workspace_ref_from_head(&self) -> Result<git2::Reference<'_>> {
        ...
        if head_ref.name_bytes() == b"refs/heads/fluxgit/workspace" {
            Ok(head_ref)
        } else {
            Err(anyhow!(
    ...
    fn commit_with_signature(
        ...
        if repo.git_settings()?.fluxgit_sign_commits.unwrap_or(false) {
            let mut buf = Vec::new();
            ...
            match signature {
                ...
                Err(err) => {
                    // If signing fails, set the "fluxgit.signCommits" config to false before erroring out
                    if repo
                        .config_snapshot()
                        .boolean_filter("fluxgit.signCommits", |md| {
                            md.source != gix::config::Source::Local
                        })
                        .is_none()
                    {
                        repo.set_git_settings(&GitConfigSettings {
                            fluxgit_sign_commits: Some(false),
                            ..Default::default()
                        })?;
                        ...
                        tracing::warn!(
                            "Commit signing failed but remains enabled as fluxgit.signCommits is explicitly enabled globally"
                        );
                        return Err(err);
                    }
                }
        ...
        if repo.git_settings()?.fluxgit_gerrit_mode.unwrap_or(false) {
