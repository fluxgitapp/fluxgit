//! Management of FluxGit-installed Git hooks
//!
//! This module handles installation and cleanup of Git hooks that prevent
//! accidental `git commit` usage on the `fluxgit/workspace` branch and
//! provide auto-cleanup when users checkout away from FluxGit mode.
...
/// Marker comment to identify FluxGit-managed hooks
const FLUXGIT_HOOK_SIGNATURE: &str = "# FLUXGIT_MANAGED_HOOK_V1";

/// Pre-commit hook script content
const PRE_COMMIT_HOOK_SCRIPT: &str = r#"#!/bin/sh
# FLUXGIT_MANAGED_HOOK_V1
# This hook is managed by FluxGit to prevent accidental commits on the workspace branch.
...
if [ "$BRANCH" = "fluxgit/workspace" ]; then
    echo ""
    echo "FLUXGIT_ERROR: Cannot commit directly to fluxgit/workspace branch."
    echo ""
    echo "FluxGit manages commits on this branch. Please use FluxGit to commit your changes:"
    echo "  - Use the FluxGit app to create commits"
    echo "  - Or run 'but commit' from the command line"
    echo ""
    echo "If you want to exit FluxGit mode and use normal git:"
    echo "  - Run 'but teardown' to switch to a regular branch"
    echo "  - Or directly checkout another branch: git checkout <branch>"
    echo ""
    echo "If you no longer have the FluxGit CLI installed, you can simply remove this hook and checkout another branch:"
...
if [ -x "$HOOKS_DIR/pre-commit-user" ]; then
    echo ""
    echo "WARNING: FluxGit's pre-commit hook is still installed but you're not on fluxgit/workspace."
    echo "If you're no longer using FluxGit, you can restore your original hook:"
...
# FLUXGIT_MANAGED_HOOK_V1
# This hook auto-cleans FluxGit hooks when you checkout away from fluxgit/workspace.
...
# If we just left fluxgit/workspace (and aren't coming back to it)
PREV_BRANCH=$(git name-rev --name-only "$PREV_HEAD" 2>/dev/null | sed 's|^remotes/||')
if echo "$PREV_BRANCH" | grep -q "fluxgit/workspace"; then
    if [ "$NEW_BRANCH" != "fluxgit/workspace" ]; then
        echo ""
        echo "NOTE: You have left FluxGit's managed workspace branch."
        echo "Cleaning up FluxGit hooks..."

        HOOKS_DIR=$(dirname "$0")

        # Restore pre-commit - but only if it's FluxGit-managed
        if [ -f "$HOOKS_DIR/pre-commit-user" ]; then
            ...
            # Only remove if it's FluxGit-managed (has our signature)
            if grep -q "FLUXGIT_MANAGED_HOOK_V1" "$HOOKS_DIR/pre-commit"; then
                rm "$HOOKS_DIR/pre-commit"
                echo "  Removed: pre-commit (FluxGit managed)"
            else
                echo "  Warning: pre-commit hook is not FluxGit-managed, leaving it untouched"
            fi
        fi
        ...
        if [ -x "$HOOKS_DIR/post-checkout-user" ]; then
            ...
            # Only remove self if we're FluxGit-managed (we should be, but check anyway)
            if grep -q "FLUXGIT_MANAGED_HOOK_V1" "$HOOKS_DIR/post-checkout"; then
                rm "$HOOKS_DIR/post-checkout"
                echo "  Removed: post-checkout (FluxGit managed)"
            else
                echo "  Warning: post-checkout hook is not FluxGit-managed, leaving it untouched"
            fi
        fi

        echo ""
        echo "To return to FluxGit mode, run: but setup"
        echo ""
        exit 0
    fi
fi
...
fn is_fluxgit_managed_hook(path: &Path) -> bool {
    if let Ok(content) = fs::read_to_string(path) {
        content.contains(FLUXGIT_HOOK_SIGNATURE)
    } else {
        false
    }
}
...
fn install_hook(hooks_dir: &Path, hook_type: ManagedHookType) -> Result<HookInstallationResult> {
    ...
    if hook_path.exists() && is_fluxgit_managed_hook(&hook_path) {
...
/// Install all FluxGit managed hooks
///
/// Called after switching HEAD to fluxgit/workspace
...
/// Uninstall all FluxGit managed hooks and restore user's originals
///
