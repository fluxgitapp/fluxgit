    ...
    Ok(
        match git2_hooks::hooks_pre_commit(&*ctx.git2_repo.get()?, husky_search_paths(ctx))? {
            ...
            H::RunNotSuccessful {
                ...
                // If the output contains FLUXGIT_ERROR, it's our managed hook blocking
                // commits on fluxgit/workspace - this is expected behavior, not a failure
                if stdout.contains("FLUXGIT_ERROR") || stderr.contains("FLUXGIT_ERROR") {
                    HookResult::Success
                } else {
                    let error = join_output(stdout, stderr, code);
...
pub fn pre_push(
    ...
    remote_tracking_branch: &fluxgit_reference::RemoteRefname,
