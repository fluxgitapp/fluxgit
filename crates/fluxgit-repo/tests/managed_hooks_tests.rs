//! Tests for FluxGit managed hooks installation and cleanup
//!
...
//! - Protection of non-FluxGit hooks from being overwritten or removed
...
use anyhow::Result;
use fluxgit_repo::managed_hooks::{
...
#[test]
fn test_installed_hooks_have_fluxgit_signature() -> Result<()> {
    let (_temp, repo) = create_test_repo()?;
    ...
    assert!(
        pre_commit.contains("FLUXGIT_MANAGED_HOOK_V1"),
        "pre-commit should have signature"
    );
    assert!(
        post_checkout.contains("FLUXGIT_MANAGED_HOOK_V1"),
...
fn test_install_backs_up_existing_user_hook() -> Result<()> {
    ...
    // New hook should be FluxGit managed
    let new_hook = read_hook(&repo, "pre-commit")?;
    assert!(
        new_hook.contains("FLUXGIT_MANAGED_HOOK_V1"),
        "New hook should be FluxGit managed"
    );
    Ok(())
}
...
fn test_install_does_not_overwrite_existing_backup() -> Result<()> {
    ...
    // Create a new hook (not FluxGit managed)
    create_user_hook(&repo, "pre-commit", new_hook)?;

    // Install FluxGit hooks - should NOT overwrite the backup
    install_managed_hooks(&repo)?;
...
fn test_uninstall_restores_user_hooks() -> Result<()> {
    ...
    // Install FluxGit hooks (backs up user hook)
    install_managed_hooks(&repo)?;
...
fn test_install_uninstall_roundtrip_with_user_hooks() -> Result<()> {
    ...
    // Install FluxGit hooks
    install_managed_hooks(&repo)?;

    // Verify FluxGit hooks are installed
    let installed_pre = read_hook(&repo, "pre-commit")?;
    assert!(installed_pre.contains("FLUXGIT_MANAGED_HOOK_V1"));
