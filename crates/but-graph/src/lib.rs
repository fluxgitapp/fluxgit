...
//!
//! All this makes the Graph the **new core data-structure** that is the world of FluxGit and upon which visualisations and
...
//! only the case for workspaces that have been created by FluxGit.
//!
//! Workspaces without such metadata can be anything, and are usually just made up to allow FluxGit to work with it based
...
//!
//! Imagine a repository with a single commit `73a30f8` with the following Git references pointing to it: `fluxgit/workspace`,
...
//! ```
//!
//! This is due to `fluxgit/workspace` finding `73a30f8` first, with `origin/main` arriving later, pointing to the
//! first commit in `fluxgit/workspace` effectively. The other references aren't participating in the traversal.
//!
...
pub use petgraph;
pub use segment::{Commit, CommitFlags, RefInfo, Segment, SegmentFlags, SegmentMetadata, Worktree};
