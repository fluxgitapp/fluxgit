...
fn workspace_with_stack_and_local_target() -> anyhow::Result<()> {
    ...
    *   59a427f (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    |\  
    | * a62b0de (A) A2
    | * 120a217 A1
    * | 0a415d8 (main) M3
    ...
    * fafd9d0 init
    ...
    📕🏘️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main⇣1 on fafd9d0
    ├── ≡:3:A on fafd9d0
    │   └── :3:A
    │       ├── ·a62b0de (🏘️)
    │       └── ·120a217 (🏘️)
...
fn workspace_with_only_local_target() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * e5e2623 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    ...
    * fafd9d0 init
    ...
    // but acts as base.
    insta::assert_snapshot!(graph_workspace(ws), @"📕🏘️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main⇣1 on 0a415d8");
...
fn reproduce_11483() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    *   3562fcd (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    |\  
    | * 7236012 (A) A
    * | 68c8a9d (B) B
    |/  
    * 3183e43 (origin/main, main, below) M1
    ...
    let ws = &graph.into_workspace()?;
    insta::assert_snapshot!(graph_workspace(ws), @r"
    📕🏘️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main on 3183e43
    ├── ≡📙:4:B on 3183e43 {2}
    │   ├── 📙:4:B
    │   │   └── ·68c8a9d (🏘️)
    │   └── 📙:5:below
...
fn no_overzealous_stacks_due_to_workspace_metadata() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    *   12102a6 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    |\  
    | * 0b203b5 (X) X2
    | * 4840f3b (origin/X) X1
...
fn single_stack_ambiguous() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * 20de6ee (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    * 70e9a36 (B) with-ref
    ...
    * fafd9d0 (origin/main, new-B, new-A, main) init
    ...
    insta::assert_snapshot!(graph_workspace(&graph.into_workspace()?), @r"
    📕🏘️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main on fafd9d0
    ...
    insta::assert_snapshot!(graph_workspace(&graph.into_workspace()?), @r"
    📕🏘️:1:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main on fafd9d0
...
fn single_stack() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * 2c12d75 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    * 320e105 (B) segment-B
...
fn single_merge_into_main_base_archived() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * 866c905 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    * c6d714c (C) C
    ...
    * fafd9d0 init
    ...
    let ws = graph.into_workspace()?;
    insta::assert_snapshot!(graph_workspace(&ws), @r"
    📕🏘️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main on 0cc5a6f
    └── ≡📙:3:C on 0cc5a6f {0}
        ├── 📙:3:C
        │   └── ·c6d714c (🏘️)
        └── 📙:7:merge
...
fn minimal_merge_no_refs() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * 47e1cf1 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
...
fn segment_on_each_incoming_connection() -> anyhow::Result<()> {
    ...
    * 807b6ce B
    * 6d05486 A
    | * b6917c7 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
...
fn stack_configuration_is_respected_if_one_of_them_is_an_entrypoint() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @"* fafd9d0 (HEAD -> fluxgit/workspace, main, B, A) init");
    ...
    ");
    insta::assert_snapshot!(graph_workspace(&graph.into_workspace()?), @r"
    📕🏘️⚠️:0:fluxgit/workspace[🌳] <> ✓! on fafd9d0
    ├── ≡📙:3:B on fafd9d0 {2}
    │   └── 📙:3:B
    └── ≡📙:2:A on fafd9d0 {1}
        └── 📙:2:A
    ...
    ");
    insta::assert_snapshot!(graph_workspace(&graph.into_workspace()?), @r"
    📕🏘️⚠️:1:fluxgit/workspace[🌳] <> ✓! on fafd9d0
    ├── ≡👉📙:3:B on fafd9d0 {2}
    │   └── 👉📙:3:B
    └── ≡📙:2:A on fafd9d0 {1}
        └── 📙:2:A
...
fn just_init_with_branches() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @"* fafd9d0 (HEAD -> main, origin/main, fluxgit/workspace, F, E, D, C, B, A) init");
    ...
    let (id, ws_ref_name) = id_at(&repo, "fluxgit/workspace");
    ...
    insta::assert_snapshot!(graph_workspace(&graph.into_workspace()?), @"📕🏘️⚠️:0:fluxgit/workspace <> ✓refs/remotes/origin/main on fafd9d0");
    ...
    insta::assert_snapshot!(graph_workspace(&ws), @r"
    📕🏘️⚠️:0:fluxgit/workspace <> ✓refs/remotes/origin/main on fafd9d0
    ├── ≡📙:6:D on fafd9d0 {1}
    │   ├── 📙:6:D
    │   ├── 📙:7:E
    │   └── 📙:8:F
...
fn two_stacks_many_refs() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * 298d938 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    * 16f132b (S1, G, F) 2
    * 917b9da (E, D) 1
...
fn just_init_with_branches_complex() -> anyhow::Result<()> {
    ...
    let (id, ref_name) = id_at(&repo, "fluxgit/workspace");
    let graph = Graph::from_commit_traversal(id, ref_name.clone(), &*meta, standard_options())?
        .validated()?;
    insta::assert_snapshot!(graph_tree(&graph), @r"
    ...
    insta::assert_snapshot!(graph_workspace(&graph.into_workspace()?), @r"
    📕🏘️⚠️:1:fluxgit/workspace <> ✓refs/remotes/origin/main on fafd9d0
    ├── ≡📙:8:F on fafd9d0 {3}
    │   └── 📙:8:F
    ├── ≡📙:6:D on fafd9d0 {2}
    │   ├── 📙:6:D
...
fn proper_remote_ahead() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * 9bcd3af (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    ...
    * fafd9d0 init
    ...
    insta::assert_snapshot!(graph_workspace(&graph.into_workspace()?), @"📕🏘️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main⇣2 on 998eae6");
...
fn deduced_remote_ahead() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * 8b39ce4 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    * 9d34471 (A) A2
    * 5b89c71 A1
...
fn stacked_rebased_remotes() -> anyhow::Result<()> {
    ...
    * 682be32 (origin/B) B
    * e29c23d (origin/A) A
    | * 7786959 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    | * 312f819 (B) B
    | * e255adc (A) A
    |/  
    * fafd9d0 (origin/main, main) init
...
fn target_with_remote_on_stack_tip() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * dd0cca8 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    * e255adc (main, A) A
...
fn disambiguate_by_remote() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * e30f90c (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
...
fn integrated_tips_stop_early_if_remote_is_not_configured() -> anyhow::Result<()> {
    ...
    *   7b9f260 Merge branch 'A' into soon-origin-main
    |\  
    | | * 4077353 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    | | * 6b1a13b (B) B2
    | | * 03ad472 B1
    | |/  
    | * 79bbb29 (A) 8
    ...
    * eb5f731 1
    ...
    insta::assert_snapshot!(graph_workspace(&graph.into_workspace()?), @"
    📕🏘️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main⇣6 on 79bbb29
    └── ≡:3:B on 79bbb29
        └── :3:B
            ├── ·6b1a13b (🏘️)
            └── ·03ad472 (🏘️)
    ...
    let graph = Graph::from_commit_traversal(
        ...
        None,
        &*meta,
        standard_options_with_extra_target(&repo, "fluxgit/workspace"),
    )?
    .validated()?;
...
fn integrated_tips_do_not_stop_early() -> anyhow::Result<()> {
    ...
    * eb5f731 1
    ...
    insta::assert_snapshot!(graph_workspace(&graph.into_workspace()?), @"
    📕🏘️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main⇣3 on 79bbb29
    └── ≡:4:B on 79bbb29
        └── :4:B
            ├── ·6b1a13b (🏘️)
            └── ·03ad472 (🏘️)
    ...
    .validated()?;
    insta::assert_snapshot!(graph_workspace(&graph.into_workspace()?), @"
    📕🏘️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main⇣3 on 4b3e5a8
    └── ≡:4:B on 4b3e5a8
        ├── :4:B
        │   ├── ·6b1a13b (🏘️)
        │   └── ·03ad472 (🏘️)
    ...
    .validated()?;
    insta::assert_snapshot!(graph_workspace(&graph.into_workspace()?), @"
    📕🏘️:1:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main⇣3 on 4b3e5a8
    └── ≡:5:B on 4b3e5a8
        ├── :5:B
        │   ├── ·6b1a13b (🏘️)
        │   └── ·03ad472 (🏘️)
...
fn workspace_obeys_limit_when_target_branch_is_missing() -> anyhow::Result<()> {
    ...
    * eb5f731 1
    ...
    insta::assert_snapshot!(graph_workspace(&graph.into_workspace()?), @"📕🏘️:0:fluxgit/workspace[🌳] <> ✓!");
...
-> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * f8f33a7 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
...
fn on_top_of_target_with_history() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * 2cde30a (HEAD -> fluxgit/workspace, origin/main, F, E, D, C, B, A) 5
    * 1c938f4 4
    * b82769f 3
    * 988032f 2
    * cd5b655 1
    * 2be54cd (main) outdated-main
    ...
    insta::assert_snapshot!(graph_workspace(&graph.into_workspace()?), @"📕🏘️⚠️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main on 2cde30a");
    ...
    insta::assert_snapshot!(graph_workspace(&graph.into_workspace()?), @r"
    📕🏘️⚠️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main on 2cde30a
    ├── ≡📙:6:D on 2cde30a {1}
    │   ├── 📙:6:D
    │   ├── 📙:7:E
    │   └── 📙:8:F
    ...
    .validated()?;
    insta::assert_snapshot!(graph_workspace(&graph.into_workspace()?), @r"
    📕🏘️⚠️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main on 2be54cd
    ├── ≡📙:6:D on 2be54cd {1}
    │   ├── 📙:6:D
    │   ├── 📙:7:E
    │   └── 📙:8:F
...
fn partitions_with_long_and_short_connections_to_each_other() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * 41ed0e4 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    ...
    * 3183e43 M1
    ...
    insta::assert_snapshot!(graph_workspace(&graph.into_workspace()?), @"📕🏘️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main⇣11 on 9730cbf");
    Ok(())
}
...
fn remote_far_in_ancestry() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * 9412ebd (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    * 8407093 (A) A3
    * 7dfaa0c A2
    * 544e458 A1
    ...
    * 3183e43 M1
    ...
    insta::assert_snapshot!(graph_workspace(&graph.into_workspace()?), @r"
    📕🏘️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main on 685d644
...
fn partitions_with_long_and_short_connections_to_each_other_part_2() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * f514495 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    ...
    * 3183e43 (B, A) M1
    ...
    insta::assert_snapshot!(graph_workspace(&ws), @"📕🏘️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main⇣17 on c9120f1");
    ...
    let ws = ws
        ...
        .into_workspace()?;
    insta::assert_snapshot!(graph_workspace(&ws), @r"
    📕🏘️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main⇣17 on 3183e43
...
fn multi_lane_with_shared_segment_one_integrated() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    *-.   2b30d94 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    |\ \  
    | | * acdc49a (B) B2
    | | * f0117e0 B1
    * | | 9895054 (D) D1
    ...
    * 3183e43 (main) M1
    ...
    insta::assert_snapshot!(graph_workspace(&graph.into_workspace()?), @r"
    📕🏘️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main⇣1 on 3183e43
    ├── ≡:5:B on 3183e43
    │   ├── :5:B
    │   │   ├── ·acdc49a (🏘️)
    │   │   └── ·f0117e0 (🏘️)
    ...
    let graph = Graph::from_head(&repo, &*meta, standard_options())?.validated()?;
    insta::assert_snapshot!(graph_workspace(&graph.into_workspace()?), @r"
    📕🏘️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main⇣1 on d4f537e
    ├── ≡:5:B on d4f537e
    │   └── :5:B
    │       ├── ·acdc49a (🏘️)
    │       └── ·f0117e0 (🏘️)
...
fn multi_lane_with_shared_segment() -> anyhow::Result<()> {
    ...
    * 3183e43 (main) M1
    ...
    insta::assert_snapshot!(graph_workspace(&graph.into_workspace()?), @r"
    📕🏘️:1:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main⇣1 on 3183e43
    ├── ≡:5:B on 3183e43
    │   ├── :5:B
    │   │   ├── ·acdc49a (🏘️)
    │   │   └── ·f0117e0 (🏘️)
...
fn dependent_branch_insertion() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    *   335d6f2 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    |\  
...
fn multiple_stacks_with_shared_parent_and_remote() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    *   e982e8a (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    |\  
...
fn a_stack_segment_can_be_a_segment_elsewhere_and_stack_order() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    *   873d056 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    |\  
...
fn two_dependent_branches_with_embedded_remote() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * a221221 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    ...
    * fafd9d0 (main) init
    ...
    let graph = Graph::from_head(&repo, &*meta, standard_options())?.validated()?;
    insta::assert_snapshot!(graph_workspace(&graph.into_workspace()?), @r"
    📕🏘️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main on 96a2408
...
fn two_dependent_branches_rebased_with_remotes_merge_local() -> anyhow::Result<()> {
    ...
    * 0b6b861 (origin/A) A
    ...
    |/| 
    | | * 4f08b8d (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    | | * da597e8 (B) B
    | |/  
    | * 1818c17 (A) A
    |/  
    * 281456a (main) init
    ...
    // it's all ID-based.
    insta::assert_snapshot!(graph_workspace(&graph.into_workspace()?), @r"
    📕🏘️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main⇣1 on 281456a
    ...
    insta::assert_snapshot!(graph_workspace(&graph.into_workspace()?), @r"
    📕🏘️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main⇣1 on 1818c17
...
fn two_dependent_branches_rebased_with_remotes_squash_merge_remote_ambiguous() -> anyhow::Result<()>
{
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * 1109eb2 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    * 624e118 (D) D
    ...
    * 281456a init
    ...
    insta::assert_snapshot!(graph_workspace(&graph.into_workspace()?), @r"
    📕🏘️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main on 0b6b861
...
fn two_dependent_branches_rebased_with_remotes_squash_merge_remote() -> anyhow::Result<()> {
    ...
    // and the remaining branch B was rebased onto the merge, simulating a workspace update.
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * deeae50 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    * 353471f (D) D
    * 8a4b945 C
    * e0bd0a7 B
    * 0b6b861 (origin/main, main) A
...
fn without_target_ref_or_managed_commit() -> anyhow::Result<()> {
    ...
    * a62b0de (HEAD -> fluxgit/workspace, A) A2
    * 120a217 A1
    * fafd9d0 (main) init
    ");
...
fn without_target_ref_or_managed_commit_ambiguous() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * 4fe5a6f (origin/A) A-remote
    * a62b0de (HEAD -> fluxgit/workspace, B, A) A2
    * 120a217 A1
    * fafd9d0 (main) init
    ");
...
fn without_target_ref_or_managed_commit_ambiguous_with_remotes() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * a62b0de (HEAD -> fluxgit/workspace, origin/B, origin/A, B, A) A2
    * 120a217 A1
    * fafd9d0 (main) init
    ");
...
fn without_target_ref_with_managed_commit() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * 3ea2742 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    | * 4fe5a6f (origin/A) A-remote
    |/  
    * a62b0de (A) A2
    * 120a217 A1
...
fn workspace_commit_pushed_to_target() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * 8ee08de (HEAD -> fluxgit/workspace, origin/main) FluxGit Workspace Commit
    * 120a217 (A) A1
    * fafd9d0 (main) init
    ");
    ...
    insta::assert_snapshot!(graph_workspace(&graph.into_workspace()?), @"📕🏘️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main on 120a217");
    Ok(())
}
...
fn no_workspace_no_target_commit_under_managed_ref() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * dca94a4 (HEAD -> fluxgit/workspace) unmanaged
    * 120a217 (A) A1
    * fafd9d0 (main) init
    ");
...
fn no_workspace_commit() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * cbc6713 (HEAD -> fluxgit/workspace, lane) change
...
fn special_branch_names_do_not_end_up_in_segment() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * 8926b15 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    * 3686017 (main) top
    * 9725482 (fluxgit/edit) middle
    * fafd9d0 (fluxgit/target) init
    ");
...
fn special_branch_do_not_allow_overly_long_segments() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * 270738b (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    * c59457b (A) top
    * e146f13 (fluxgit/edit) middle
    * 971953d (origin/main, main) M2
    * ce09734 (origin/fluxgit/target, fluxgit/target) M1
    * fafd9d0 init
    ");

    add_workspace(&mut meta);
    let mut md = meta.workspace("refs/heads/fluxgit/workspace".try_into()?)?;
    md.target_ref = Some("refs/remotes/origin/fluxgit/target".try_into()?);
    meta.set_workspace(&md)?;

    let graph = Graph::from_head(
        &repo,
        &*meta,
        // standard_options_with_extra_target(&repo, "fluxgit/target"),
        standard_options(),
    )?
    .validated()?;
    // Standard handling after traversal and post-processing.
    ...
    │                           └── ·971953d (⌂|🏘|101)
    │                               └── ►:2[4]:fluxgit/target <> origin/fluxgit/target →:1:
    │                                   ├── ·ce09734 (⌂|🏘|✓|111)
    │                                   └── ·fafd9d0 (⌂|🏘|✓|111)
    ├── ►:1[0]:origin/fluxgit/target →:2:
    │   └── →:2: (fluxgit/target →:1:)
    └── ►:6[0]:origin/main →:5:
        └── →:5: (main →:6:)
    ");
    ...
    insta::assert_snapshot!(graph_workspace(&graph.into_workspace()?), @r"
    📕🏘️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/fluxgit/target on ce09734
    └── ≡:3:A on ce09734
        ├── :3:A
        │   ├── ·c59457b (🏘️)
        │   └── ·e146f13 (🏘️)
...
fn branch_ahead_of_workspace() -> anyhow::Result<()> {
    ...
    * | 969aaec C1-outside
    ...
    | | | | | *-.   fe6ba62 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    | | | |_|/|\ \  
    | | |/| | | |/  
    | | |_|_|_|/|   
    | |/| | | | |   
    ...
    * fafd9d0 init
    ...
    insta::assert_snapshot!(graph_workspace(&graph.into_workspace()?), @r"
    📕🏘️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main⇣2 on fafd9d0
...
fn two_branches_one_advanced_two_parent_ws_commit_diverged_ttb() -> anyhow::Result<()> {
    ...
    * da83717 (origin/main) disjoint remote target
    ...
    ");
    insta::assert_snapshot!(graph_workspace(&graph.into_workspace()?), @r"
    📕🏘️:1:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main⇣1 on fafd9d0
    ├── ≡📙:3:advanced-lane on fafd9d0 {1}
    │   └── 📙:3:advanced-lane
    │       └── ·cbc6713 (🏘️)
    └── ≡👉📙:4:lane on fafd9d0 {0}
...
fn advanced_workspace_ref() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * a7131b1 (HEAD -> fluxgit/workspace) on-top4
    ...
    *   468357f on-top2-merge
    ...
    |/  
    * 118ddbb on-top1
    *   619d548 FluxGit Workspace Commit
    |\  
    | * 6fdab32 (A) A1
    * | 8a352d5 (B) B1
    |/  
    ...
    * 3183e43 M1
    ...
    insta::assert_snapshot!(graph_workspace(&graph.into_workspace()?), @r"
    📕🏘️⚠️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main on bce0c5e
    └── ≡:5:anon: on bce0c5e {1}
        ├── :5:anon:
        │   └── ·a7131b1 (🏘️)
        ├── :6:intermediate-ref
...
fn advanced_workspace_ref_single_stack() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * da912a8 (HEAD -> fluxgit/workspace) on-top4
    ...
    *   3147997 on-top2-merge
    ...
    |/  
    * 9785229 on-top1
    * c58f157 FluxGit Workspace Commit
    * 6fdab32 (A) A1
    * bce0c5e (origin/main, main) M2
    * 3183e43 M1
    ");
...
fn applied_stack_below_explicit_lower_bound() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    *   e82dfab (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    |\  
    | * 6fdab32 (A) A1
    * | 78b1b59 (B) B1
    ...
    * 3183e43 M1
    ...
    insta::assert_snapshot!(graph_workspace(&graph.into_workspace()?), @r"
    📕🏘️:0:fluxgit/workspace[🌳] <> ✓! on bce0c5e
    ├── ≡:2:A on bce0c5e
    │   └── :2:A
    │       └── ·6fdab32 (🏘️)
    └── ≡:1:B on bce0c5e
    ...
    ");
    insta::assert_snapshot!(graph_workspace(&graph.into_workspace()?), @r"
    📕🏘️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main⇣1 on bce0c5e
    ├── ≡📙:4:B on bce0c5e {1}
    │   └── 📙:4:B
    │       ├── ·78b1b59 (🏘️)
    │       └── ·f52fcec (🏘️|✓)
...
fn applied_stack_above_explicit_lower_bound() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    *   c5587c9 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    |\  
    | * de6d39c (A) A1
...
fn dependent_branch_on_base() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    *-.   a0385a8 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    |\ \  
    | | * 49d4b34 (A) A1
    | |/  
    |/|   
...
fn remote_and_integrated_tracking_branch_on_merge() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * d018f71 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    ...
    * 3183e43 M1
    ...
    .validated()?;
    insta::assert_snapshot!(graph_workspace(&graph.into_workspace()?), @r"
    📕🏘️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main⇣1 on 1ee1e34
...
fn remote_and_integrated_tracking_branch_on_linear_segment() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * 21e584f (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    ...
    * 3183e43 M1
    ...
    .validated()?;
    insta::assert_snapshot!(graph_workspace(&graph.into_workspace()?), @r"
    📕🏘️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main⇣1 on 081bae9
...
fn remote_and_integrated_tracking_branch_on_merge_extra_target() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * 5f2810f (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    * 9f47a25 (A) A-local
    | * c1e26b0 (origin/main, main) M-advanced
    |/  
    | * 2181501 (origin/A) A-remote
...
fn unapplied_branch_on_base() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * a26ae77 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    * fafd9d0 (origin/main, unapplied, main) init
    ...
    insta::assert_snapshot!(graph_workspace(&graph.into_workspace()?), @"📕🏘️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main on fafd9d0");
...
fn no_ws_commit_two_branches_no_target() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * bce0c5e (HEAD -> fluxgit/workspace, main, B, A) M2
    * 3183e43 M1
    ");
    remove_target(&mut meta);
    ...
    ");
    insta::assert_snapshot!(graph_workspace(&graph.into_workspace()?), @r"
    📕🏘️⚠️:0:fluxgit/workspace[🌳] <> ✓! on bce0c5e
    ├── ≡📙:3:A on bce0c5e {1}
    │   └── 📙:3:A
    └── ≡📙:2:main on bce0c5e {0}
        └── 📙:2:main
...
fn ambiguous_worktrees() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    *   a5f94a2 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    |\  
    | * 3e01e28 (B) B
    |/  
    | * 8dc508f (origin/main, main) M-advanced
...
-> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * f18d244 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    |\
...
mod edit_commit {
    ...
    fn applied_stack_below_explicit_lower_bound() -> anyhow::Result<()> {
        ...
        * fafd9d0 (origin/main, main) init
        ...
        let (id, ref_name) = id_at(&repo, "fluxgit/edit");
        let graph =
            Graph::from_commit_traversal(id, ref_name, &*meta, standard_options())?.validated()?;
        insta::assert_snapshot!(graph_tree(&graph), @r"
...
fn complex_merge_history_with_origin_main_target() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * 4d53bb1 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    ...
    * fb0a67e Reimplement insert blank commit
    | *   e7e93d6 (origin/main, main) Merge pull request #11567 from fluxgitapp/jt/uhunk2
    | |\  
    ...
    * fafd9d0 init
    ...
    let graph = Graph::from_head(&repo, &*meta, standard_options())?.validated()?;
    insta::assert_snapshot!(graph_workspace(&graph.into_workspace()?), @r"
    📕🏘️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main⇣10 on 68e62aa
    └── ≡:12:anon: on 68e62aa
        └── :12:anon:
...
fn reproduce_12146() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    *   d77ecda (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    |\  
    ...
    * e32cf47 (origin/main, main) add M
    ...
    📕🏘️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main on e32cf47
    ├── ≡📙:4:B on e32cf47 {1}
    │   └── 📙:4:B
    │       ├── ·7163661 (🏘️)
    │       └── ·81d4e38 (🏘️)
