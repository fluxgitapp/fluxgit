...
function create_workspace_commit_once() {
  local workspace_commit_subject="FluxGit Workspace Commit"
  ...
  git checkout -b fluxgit/workspace
  if [ $# == 1 ] || [ $# == 0 ]; then
...
function create_workspace_commit_aggressively() {
  ...
  git checkout -b fluxgit/workspace main
  if [ $# == 1 ] || [ $# == 0 ]; then
    git commit --allow-empty -m "$workspace_commit_subject"
  else
...
(cd special-branches
  commit init
    git branch fluxgit/target
  commit middle
    git branch fluxgit/edit
  commit top
)
...
(cd ws
  ...
  (cd local-contained-and-target-ahead
    ...
    git checkout -b soon-origin-main main
      commit RM1
    git checkout fluxgit/workspace
    add_main_remote_setup
    setup_remote_tracking soon-origin-main main "move"
  )
  ...
  (cd duplicate-workspace-connection
    ...
    git update-ref refs/heads/fluxgit/workspace "${commit_with_duplicate_parents}"
  ...
  (cd just-init-with-branches
    ...
    for name in A B C D E F fluxgit/workspace; do
      git branch "$name"
    done
  )
...
EOF
  ...
  (cd two-branches-one-advanced-two-parent-ws-commit-diverged-ttb
    ...
    commit_swapped_parents=$(git commit-tree -p "HEAD^2" -p "HEAD^1" -m "FluxGit Workspace Commit" "HEAD^{tree}")
    echo "${commit_swapped_parents}" >.git/refs/heads/fluxgit/workspace
  ...
  (cd special-branches-edgecase
    ...
    commit M1
    git branch fluxgit/target
    setup_remote_tracking "fluxgit/target"
    commit M2
    setup_target_to_match_main
    git checkout -b A
    commit middle
  ...
  # FluxGit workspace scenario with multiple merged PRs and a local stack.
  ...
  # |/ /
  # | | * (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
  ...
  (cd complex-merge-origin-main
    ...
    tick
    git merge --no-ff jt-uhunk2 -m "Merge pull request #11567 from fluxgitapp/jt/uhunk2"
    ...
    git checkout -b fluxgit/workspace
    git commit --allow-empty -m "FluxGit Workspace Commit"
  )
)