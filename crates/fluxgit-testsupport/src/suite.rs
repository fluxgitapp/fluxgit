...
impl Suite {
    ...
    pub fn sign_in(&self) -> fluxgit_user::User {
        crate::secrets::setup_blackhole_store();
        let user: fluxgit_user::User =
            serde_json::from_str(include_str!("fixtures/user/minimal.v1"))
                .expect("valid v1 user file");
        fluxgit_user::set_user(&user).expect("failed to add user");
        user
    }

    fn project(&self, fs: HashMap<PathBuf, &str>) -> (fluxgit_project::Project, TempDir) {
        let (repo, tmp) = test_repository();
        for (path, contents) in fs {
            if let Some(parent) = path.parent() {
                fs::create_dir_all(repo.path().parent().unwrap().join(parent))
                    .expect("failed to create dir");
            }
            fs::write(
                repo.path().parent().unwrap().join(&path),
                contents.as_bytes(),
            )
        ...
        let outcome = fluxgit_project::add_at_app_data_dir(
            self.local_app_data(),
...
pub struct Case {
    pub project: fluxgit_project::Project,
    pub ctx: Context,
...
impl Case {
    fn new(project: fluxgit_project::Project, project_tmp: TempDir) -> Case {
    ...
    pub fn refresh(mut self, _suite: &Suite) -> Self {
        let project = fluxgit_project::get(self.project.id).expect("failed to get project");
        let ctx = Context::new_from_legacy_project_and_settings(&project, AppSettings::default());
        Self {
            ctx,
            project,
