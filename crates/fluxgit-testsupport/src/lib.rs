pub const VAR_NO_CLEANUP: &str = "FLUXGIT_TESTS_NO_CLEANUP";
...
pub mod virtual_branches {
    use but_ctx::Context;
    use fluxgit_stack::{Target, VirtualBranchesHandle};
...
pub mod writable {
    ...
    pub fn fixture_project(
        ...
        let project = Project::new_for_fluxgit_testsupport(
    ...
    /// Use the `but` CLI instead of `fluxgit-cli` for fixtures that need stacking support.
    pub fn but_fixture(
        script_name: &str,
        project_directory: &str,
    ) -> anyhow::Result<(Context, TempDir)> {
...
pub mod read_only {
    ...
    /// and make the command-line application available to it. That way the script can perform FluxGit
    ...
    pub fn fixture_project(script_name: &str, project_directory: &str) -> anyhow::Result<Project> {
        ...
        let project = if was_inserted {
            ...
            let outcome =
                fluxgit_project::add_at_app_data_dir(tmp.path(), project_worktree_dir.as_path())?;
            outcome.try_project()?
        } else {
            Project::new_for_fluxgit_testsupport(
                project_directory.to_owned(),
...
pub(crate) static DRIVER: Lazy<PathBuf> = Lazy::new(|| {
    ...
    let res = cargo
        .args(["build", "-p=fluxgit-cli", "--features=testing"])
        .status()
