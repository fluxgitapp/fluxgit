...
fn exclusive_lock_prevents_second_acquisition() -> anyhow::Result<()> {
    ...
    assert!(
        err_msg.contains("already opened for writing by another FluxGit instance"),
...
fn background_refresh_lock_prevents_second_background_refresh() -> anyhow::Result<()> {
    ...
    let err_msg = result.err().unwrap().to_string();
    assert!(
        err_msg.contains("already being refreshed in the background by another FluxGit instance"),
