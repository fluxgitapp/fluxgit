...
pub enum LockScope {
    /// Represents a lock on the entire project, preventing other FluxGit instances from operating on it.
    #[default]
    AllOperations,
    /// Represents a lock on background refresh operations only. This would prevent other FluxGit instances
...
/// - [`LockScope::AllOperations`]: Prevents other FluxGit instances from performing any operations on the project.
...
///   user-driven operations to proceed. This enables multiple FluxGit instances to work on the same project
    ...
    if !got_lock {
        let error_message = match scope {
            LockScope::AllOperations => {
                format!(
                    "Project at '{}' is already opened for writing by another FluxGit instance",
                    project_data.display()
                )
            }
            LockScope::BackgroundRefreshOperations => {
                format!(
                    "Project at '{}' is already being refreshed in the background by another FluxGit instance",
                    project_data.display()
                )
            }
        };
...
/// FluxGit are able to read or write the same repository.
