...
pub struct Headers {
    ...
    /// forwards when you rebase a commit in FluxGit.
...
impl Headers {
    ...
    /// 1. Read `fluxgit.testing.changeId` from `config` and if it's a valid u128 integer, use it as change-id.
    /// 2. generate a new change-id
    pub fn from_config(config: &gix::config::Snapshot) -> Self {
        Headers {
            change_id: Some(
                config
                    .integer("fluxgit.testing.changeId")
                    .and_then(|id| {
                        u128::try_from(id)
                            .ok()
...
const HEADERS_VERSION_FIELD: &str = "fluxgit-headers-version";
const HEADERS_CHANGE_ID_FIELD: &str = "fluxgit-change-id";
const HEADERS_NEW_CHANGE_ID_FIELD: &str = "change-id";
/// The name of the header field that stores the amount of conflicted files.
pub const HEADERS_CONFLICTED_FIELD: &str = "fluxgit-conflicted";
