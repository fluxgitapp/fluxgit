#![deny(missing_docs)]
//! The basic primitives that FluxGit is built around.
//!
...
//!    - Try to avoid doing read-only filesystem fixtures with `tempdir`, instead use `fluxgit-testtools::readonly`.
...
//!    - don't introduce dependencies towards legacy crates (e.g. prefixed withn "fluxgit-")
...
//!    - If only one part of the application is supposed to change the worktree, let's protect the Application from itself by using `fluxgit::access` just like we do now.
...
//! * **Workspace**
//!     - A FluxGit concept of the combination of one or more branches into one worktree. This allows
...
/// Return `true` if `ref_name` looks like the standard FluxGit workspace.
///
...
pub fn is_workspace_ref_name(ref_name: &FullNameRef) -> bool {
    ref_name.as_bstr() == "refs/heads/fluxgit/workspace"
        || ref_name.as_bstr() == "refs/heads/fluxgit/integration"
}
...
/// A decoded commit object with easy access to additional FluxGit information.
#[derive(Debug, Clone)]
...
/// A decoded commit object with easy access to additional FluxGit information, without repo reference.
#[derive(Debug, Clone)]
...
pub enum Reference {
    ...
    /// A reference not visible in Git, managed by FluxGit.
