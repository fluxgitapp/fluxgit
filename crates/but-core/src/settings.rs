...
pub mod git {
    ...
    const FLUXGIT_SIGN_COMMITS: &str = "fluxgit.signCommits";
    const FLUXGIT_GERRIT_MODE: &str = "fluxgit.gerritMode";
    const FLUXGIT_FORGE_TEMPLATE_PATH: &str = "fluxgit.forgeReviewTemplatePath";
    const FLUXGIT_GITLAB_PROJECT_ID: &str = "fluxgit.gitlabProjectId";
    const FLUXGIT_GITLAB_UPSTREAM_PROJECT_ID: &str = "fluxgit.gitlabUpstreamProjectId";
    ...
    pub mod ui {
        ...
        pub struct GitConfigSettings {
            ...
            pub fluxgit_sign_commits: Option<bool>,
            pub fluxgit_gerrit_mode: Option<bool>,
            #[cfg_attr(feature = "export-ts", ts(type = "string | null"))]
            pub fluxgit_forge_review_template_path: Option<BStringForFrontend>,
            pub fluxgit_gitlab_project_id: Option<String>,
            pub fluxgit_gitlab_upstream_project_id: Option<String>,
            #[cfg_attr(feature = "export-ts", ts(type = "string | null"))]
        ...
        impl From<crate::GitConfigSettings> for GitConfigSettings {
            fn from(
                crate::GitConfigSettings {
                    fluxgit_sign_commits,
                    fluxgit_gerrit_mode,
                    fluxgit_forge_review_template_path,
                    fluxgit_gitlab_project_id,
                    fluxgit_gitlab_upstream_project_id,
                    signing_key,
                    signing_format,
                    gpg_program,
                    gpg_ssh_program,
                ...
                GitConfigSettings {
                    fluxgit_sign_commits,
                    fluxgit_gerrit_mode,
                    fluxgit_forge_review_template_path: fluxgit_forge_review_template_path
                        .map(Into::into),
                    fluxgit_gitlab_project_id,
                    fluxgit_gitlab_upstream_project_id,
    ...
    pub(crate) mod types {
        ...
        ///
        /// Some are specific to FluxGit.
        #[derive(Debug, PartialEq, Clone, Default)]
        pub struct GitConfigSettings {
            /// If `true` FluxGit should sign commits.
            /// This value is always set when querying it:
            /// * if `fluxgit.signCommits` is set, this value takes precedence over
            /// * `commit.gpgsign` which is otherwise valid.
            /// * otherwise it defaults to `false` just like Git would.
            pub fluxgit_sign_commits: Option<bool>,
            /// If `true`, FluxGit will create ChangeId trailers and will push references in the Gerrit way
            pub fluxgit_gerrit_mode: Option<bool>,
            /// The path to the review description template to be used for this repository.
            pub fluxgit_forge_review_template_path: Option<BString>,
            ...
            /// In the case of a fork, this is the project ID of the parent project, otherwise it is the same as `fluxgit_gitlab_project_id`.
            pub fluxgit_gitlab_upstream_project_id: Option<String>,
            /// `user.signingKey`.
    ...
    impl GitConfigSettings {
        ...
        pub fn try_from_snapshot(config: &gix::config::Snapshot<'_>) -> anyhow::Result<Self> {
            ...
            let fluxgit_sign_commits = config
                .boolean(FLUXGIT_SIGN_COMMITS)
                .or_else(|| config.boolean(GIT_SIGN_COMMITS))
                .or(Some(false));
            let fluxgit_gerrit_mode = config.boolean(FLUXGIT_GERRIT_MODE).or(Some(false));
            let fluxgit_forge_review_template_path = config
                .string(FLUXGIT_FORGE_TEMPLATE_PATH)
                .map(Cow::into_owned);
            let fluxgit_gitlab_project_id = config
                .string(FLUXGIT_GITLAB_PROJECT_ID)
                .and_then(string_or_ignore);
            let fluxgit_gitlab_upstream_project_id = config
                .string(FLUXGIT_GITLAB_UPSTREAM_PROJECT_ID)
                .and_then(string_or_ignore);
        ...
        pub fn persist_to_local_config(&self, repo: &gix::Repository) -> Result<()> {
            ...
            if let Some(sign_commits) = self.fluxgit_sign_commits {
                config.set_raw_value(
                    &FLUXGIT_SIGN_COMMITS,
                    if sign_commits { "true" } else { "false" },
                )?;
            };
            if let Some(gerrit_mode) = self.fluxgit_gerrit_mode {
                config.set_raw_value(
                    &FLUXGIT_GERRIT_MODE,
                    if gerrit_mode { "true" } else { "false" },
                )?;
            };
            if let Some(forge_template_path) = &self.fluxgit_forge_review_template_path {
                config.set_raw_value(
                    &FLUXGIT_FORGE_TEMPLATE_PATH,
            ...
            if let Some(gitlab_project_id) = self.fluxgit_gitlab_project_id.as_deref() {
                config.set_raw_value(&FLUXGIT_GITLAB_PROJECT_ID, gitlab_project_id)?;
            }
            if let Some(gitlab_upstream_project_id) =
                self.fluxgit_gitlab_upstream_project_id.as_deref()
            {
                config.set_raw_value(
                    &FLUXGIT_GITLAB_UPSTREAM_PROJECT_ID,
