...
/// Easy access of settings relevant to FluxGit for retrieval and storage in Git settings.
...
impl RepositoryExt for gix::Repository {
    ...
    fn commit_signatures(&self) -> anyhow::Result<(gix::actor::Signature, gix::actor::Signature)> {
        ...
        let commit_as_fluxgit = self
            .config_snapshot()
            .boolean("fluxgit.fluxgitCommitter")
            .unwrap_or_default();
        let committer = if commit_as_fluxgit {
            committer_signature()
        } else {
            self.committer()
                .transpose()?
...
const FLUXGIT_COMMIT_AUTHOR_NAME: &str = "FluxGit";
const FLUXGIT_COMMIT_AUTHOR_EMAIL: &str = "fluxgit@fluxgit.com";

/// Provide a signature with the FluxGit author, and the current time or the time overridden
