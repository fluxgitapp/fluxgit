...
/// or removing the signature if FluxGit is not configured to keep it.
///
...
pub fn create(
    ...
    if settings.fluxgit_gerrit_mode.unwrap_or(false) {
        but_gerrit::set_trailers(&mut commit);
    }
    if sign_if_configured && settings.fluxgit_sign_commits.unwrap_or(false) {
        let mut buf = Vec::new();
