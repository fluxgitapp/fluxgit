...
//! - `fluxgit-created-from`: The git reference this worktree was created from
//! - `fluxgit-base`: The base commit OID for cherry-picking
...
const CREATED_FROM_FILE: &str = "fluxgit-created-from";
...
const BASE_FILE: &str = "fluxgit-base";
...
pub fn save_worktree_meta(main_repo: &gix::Repository, worktree: WorktreeMeta) -> Result<()> {
    ...
    if let Some(ref created_from) = worktree.created_from_ref {
        std::fs::write(git_dir.join(CREATED_FROM_FILE), created_from.as_bstr())
            .context("Failed to write fluxgit-created-from file")?;
    }
    ...
    .context("Failed to write fluxgit-base file")?;
...
pub fn get_worktree_meta(repo: &gix::Repository, id: &WorktreeId) -> Result<Option<WorktreeMeta>> {
    ...
    // Read base commit
    let base_bytes = std::fs::read(&base_file).context("Failed to read fluxgit-base file")?;
    ...
    let created_from_ref = if created_from_file.exists() {
        let ref_bytes = std::fs::read(&created_from_file)
            .context("Failed to read fluxgit-created-from file")?;
...
/// List all worktrees with FluxGit metadata.
