...
use fluxgit_branch::{self, BranchCreateRequest, FLUXGIT_WORKSPACE_REFERENCE};
use fluxgit_commit::commit_ext::CommitMessageBstr as _;
use fluxgit_operating_modes::OPEN_WORKSPACE_REFS;
use fluxgit_repo::{
    RepositoryExt, SignaturePurpose,
    logging::{LogUntil, RepositoryExt as _},
};
use fluxgit_stack::{Stack, VirtualBranchesHandle};
...
pub const FLUXGIT_WORKSPACE_COMMIT_TITLE: &str = "FluxGit Workspace Commit";

// Before switching the user to our fluxgit workspace branch we save
...
pub fn update_workspace_commit(
    ...
    if let Some(branch) = &prev_branch
        && branch.head != FLUXGIT_WORKSPACE_REFERENCE.to_string()
    {
        // we are moving from a regular branch to our fluxgit workspace branch, write a file to
    ...
    let mut message = FLUXGIT_WORKSPACE_COMMIT_TITLE.to_string();
    message.push_str("\n\n");
    ...
    message.push_str(
        "Due to FluxGit managing multiple virtual branches, you cannot switch back and\n",
    );
    message.push_str("forth between git branches and virtual branches easily. \n\n");

    message.push_str("If you switch to another branch, FluxGit will need to be reinitialized.\n");
    message.push_str("If you commit on this branch, FluxGit will throw it away.\n\n");
    if !virtual_branches.is_empty() {
    ...
    message.push_str("https://docs.fluxgit.com/features/branch-management/integration-branch\n");

    let committer = fluxgit_repo::signature(SignaturePurpose::Committer)?;
    let author = fluxgit_repo::signature(SignaturePurpose::Author)?;
    ...
    repo.reference(
        &FLUXGIT_WORKSPACE_REFERENCE.clone().to_string(),
        final_commit,
        true,
        "updated workspace commit",
    )?;
    repo.set_head(&FLUXGIT_WORKSPACE_REFERENCE.clone().to_string())?;

    // Install managed hooks to prevent accidental git commits on workspace branch
    if let Err(e) = fluxgit_repo::managed_hooks::install_managed_hooks(repo) {
...
fn verify_head_is_set(ctx: &Context) -> Result<()> {
        ...
        .name()
    {
        ...
        None => Err(anyhow!(
            ...
            FLUXGIT_WORKSPACE_REFERENCE.branch()
        )),
    }
}
    ...
    let workspace_index = commits
        ...
        .position(|commit| {
            commit.message().is_some_and(|message| {
                message.starts_with(FLUXGIT_WORKSPACE_COMMIT_TITLE)
                    || message.starts_with(FLUXGIT_INTEGRATION_COMMIT_TITLE)
            })
        })
        .context("FluxGit workspace commit not found")?;
