...
use fluxgit_branch::FLUXGIT_WORKSPACE_REFERENCE;
...
use fluxgit_stack::{Stack, Target, VirtualBranchesHandle, canned_branch_name};
use serde::Serialize;
...
fn go_back_to_integration(ctx: &Context, default_target: &Target) -> Result<BaseBranch> {
    ...
    if ctx.settings.feature_flags.cv3 {
        ...
        if outcome.has_unresolved_conflicts(conflict_kind) {
            return Err(anyhow!("Conflicts while going back to fluxgit/workspace"))
...
pub(crate) fn set_base_branch(
    ...
    if !head_name
        .to_string()
        .eq(&FLUXGIT_WORKSPACE_REFERENCE.to_string())
    {
...
fn set_exclude_decoration(ctx: &Context) -> Result<()> {
    ...
    config
        .set_multivar("log.excludeDecoration", "refs/fluxgit", "refs/fluxgit")
