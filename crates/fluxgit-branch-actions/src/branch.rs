...
use fluxgit_branch::{BranchIdentity, ReferenceExtGix};
use fluxgit_reference::{RemoteRefname, normalize_branch_name};
use fluxgit_stack::{StackId, Target};
...
pub fn list_branches(
    ...
    let stacks = {
        if let Some(workspace_ref) = repo.try_find_reference("refs/heads/fluxgit/workspace")? {
            ...
            info.stacks
                .into_iter()
                .filter_map(|s| FluxGitStack::try_new(s, &remote_names).transpose())
...
fn combine_branches(
    ...
    for branch in group_branches {
        ...
        // Skip branches that should not be listed, e.g. the fluxgit technical branches like 'fluxgit/workspace'
...
struct FluxGitStack {
    ...
    // The tip comes first.
    unarchived_segments: Vec<FluxGitStackSegment>,
}

#[derive(Debug, Clone)]
struct FluxGitStackSegment {
    ...
    fn try_new(
        ...
        Ok(Some(FluxGitStack {
            ...
            name: stack
                ...
                // Hack it - the datastructure isn't suitable and this needs a `fluxgit->but` port.
                .unwrap_or_default(),
...
/// This excludes the target branch as well as fluxgit specific branches.
fn should_list_git_branch(identity: &BranchIdentity) -> bool {
    // Exclude fluxgit technical branches (not useful for the user)
    const TECHNICAL_IDENTITIES: &[&[u8]] = &[
        b"fluxgit/edit",
        b"fluxgit/workspace",
        b"fluxgit/integration", // Remove me after transition.
        b"fluxgit/target",
        b"fluxgit/oplog",
        b"HEAD",
    ];
