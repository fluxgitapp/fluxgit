...
fn workdir_vbranch_restore() -> anyhow::Result<()> {
    ...
    let mut guard = ctx.exclusive_worktree_access();
    let _empty = fluxgit_branch_actions::create_virtual_branch(
        ctx,
        &Default::default(),
        guard.write_permission(),
    )?;
...
fn basic_oplog() -> anyhow::Result<()> {
    ...
    let mut guard = ctx.exclusive_worktree_access();
    let _empty_branch_id = fluxgit_branch_actions::create_virtual_branch(
        ctx,
        &BranchCreateRequest::default(),
        guard.write_permission(),
    )?;
...
#[test]
fn restores_fluxgit_workspace() -> anyhow::Result<()> {
    let Test { repo, ctx, .. } = &mut Test::default();
    ...
    let commit1_id = {
        ...
        assert_eq!(message, FLUXGIT_WORKSPACE_COMMIT_TITLE);
        commit1_id
    };
...
fn head_corrupt_is_recreated_automatically() {
    ...
    let oplog_path = repo.path().join(".git/fluxgit/operations-log.toml");
    fs::write(
        oplog_path,
