...
fn head() {
    ...
    fluxgit_branch_actions::update_commit_message(
        ctx,
        stack_entry.id,
        commit_three_oid,
...
fn forcepush_allowed() {
    ...
    fluxgit_project::update_with_path(
    ...
    fluxgit_branch_actions::stack::push_stack(
        ctx,
        stack_entry.id,
        false,
        false,
...
fn root() {
    ...
    let mut guard = ctx.exclusive_worktree_access();
    let branch_id = fluxgit_branch_actions::create_virtual_branch(
        ctx,
        &BranchCreateRequest::default(),
        guard.write_permission(),
    )
...
fn empty() {
    ...
    assert_eq!(
        fluxgit_branch_actions::update_commit_message(ctx, branch_id.id, commit_one_oid, "",)
            .unwrap_err()
            .to_string(),
