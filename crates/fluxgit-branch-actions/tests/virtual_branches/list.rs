...
mod util {
    use anyhow::Result;
    use but_ctx::Context;
    use fluxgit_branch::BranchIdentity;
    use fluxgit_branch_actions::{BranchListing, BranchListingFilter};
    ...
    pub fn assert_equal(
        BranchListing {
            ...
            updated_at: _,
            head: _, // NOTE: can't have stable commits while `fluxgit-change-id` is not stable/is a UUID.
            last_commiter: _,
            has_local,
        }: &BranchListing,
    ...
    pub fn project_ctx(name: &str) -> Result<Context> {
        fluxgit_testsupport::read_only::fixture("for-listing.sh", name)
    }
    ...
    pub fn list_branches(
        ...
        let mut branches = fluxgit_branch_actions::list_branches(ctx, filter, None)?;
