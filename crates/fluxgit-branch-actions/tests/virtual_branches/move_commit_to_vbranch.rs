...
fn no_diffs() {
    ...
    let mut guard = ctx.exclusive_worktree_access();
    let target_stack_entry = fluxgit_branch_actions::create_virtual_branch(
        ctx,
        &BranchCreateRequest::default(),
        guard.write_permission(),
    )
    ...
    fluxgit_branch_actions::move_commit(ctx, target_stack_entry.id, commit_oid, source_branch_id)
        .unwrap();
...
fn no_branch() {
    ...
    assert_eq!(
        fluxgit_branch_actions::move_commit(ctx, id, commit_oid, source_branch_id)
            .unwrap_err()
            .to_string(),
