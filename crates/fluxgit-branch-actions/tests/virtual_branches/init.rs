...
fn twice() {
    ...
    {
        let project = fluxgit_project::add_at_app_data_dir(data_dir.path(), test_project.path())
        ...
        fluxgit_project::delete_with_path(data_dir.path(), project.id).unwrap();
    }

    {
        ...
        // even though project is on fluxgit/workspace, we should not import it
        drop(guard);
        let stacks = stack_details(&ctx);
        assert_eq!(stacks.len(), 0);
    }
...
fn dirty_target() {
    ...
    unsafe { std::env::set_var("GIT_AUTHOR_NAME", "FluxGit") };
    let mut guard = ctx.exclusive_worktree_access();
    fluxgit_branch_actions::set_base_branch(
        ctx,
        &"refs/remotes/origin/master".parse().unwrap(),
...
fn commit_on_target() {
    ...
    let old = std::env::var("GIT_AUTHOR_NAME").ok();
    unsafe {
        std::env::set_var("GIT_AUTHOR_NAME", "FluxGit");
    }
...
fn submodule() {
    ...
    let test_project = TestProject::default();
    let submodule_url: fluxgit_url::Url =
