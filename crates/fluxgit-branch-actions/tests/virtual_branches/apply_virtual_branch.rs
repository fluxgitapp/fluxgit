...
use fluxgit_branch::BranchCreateRequest;
use fluxgit_branch_actions::upstream_integration::{BranchStatus, StackStatuses, TreeStatus};
use fluxgit_reference::Refname;
use fluxgit_testsupport::stack_details;
...
fn rebase_commit() {
    ...
    fluxgit_branch_actions::set_base_branch(
        ctx,
    ...
    let mut stack_1_id = {
        ...
        let mut guard = ctx.exclusive_worktree_access();
        let stack_entry_1 = fluxgit_branch_actions::create_virtual_branch(
            ctx,
    ...
    let unapplied_branch = {
        ...
        let mut guard = ctx.exclusive_worktree_access();
        let unapplied_branch = fluxgit_branch_actions::unapply_stack(
            ctx,
            guard.write_permission(),
            stack_1_id,
            Vec::new(),
    ...
    {
        // fetch remote
        fluxgit_branch_actions::integrate_upstream(ctx, &[], None, &Default::default()).unwrap();
...
fn upstream_integration_status_without_review_map() {
    ...
    let stack_id = {
        let mut guard = ctx.exclusive_worktree_access();
        let stack_entry = fluxgit_branch_actions::create_virtual_branch(
            ctx,
            &BranchCreateRequest {
    ...
    let statuses =
        fluxgit_branch_actions::upstream_integration_statuses(ctx, None, &empty_review_map)
            .unwrap();
...
fn upstream_integration_status_with_merged_pr() {
    ...
    let statuses =
        fluxgit_branch_actions::upstream_integration_statuses(ctx, None, &review_map).unwrap();
