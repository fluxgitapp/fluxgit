...
use git2::Oid;
use fluxgit_branch_actions::{StackOrder, reorder::SeriesOrder, reorder_stack};
use fluxgit_stack::VirtualBranchesHandle;
use fluxgit_testsupport::testing_repository::assert_commit_tree_matches;
...
fn vb_commits(ctx: &Context) -> Vec<Vec<(git2::Oid, String, bool, u128)>> {
    let details = fluxgit_testsupport::stack_details(ctx);
...
fn command_ctx(name: &str) -> Result<(Context, TempDir)> {
    fluxgit_testsupport::writable::fixture_with_settings("reorder.sh", name, |_settings| {})
}
