...
const UPDATES_CHECK_URL: &str = "https://app.fluxgit.com/updates";
const REQUEST_TIMEOUT: Duration = Duration::from_secs(30);

/// Identifies which FluxGit application variant to check for updates.
///
...
pub enum AppName {
    ...
    ///
    /// This is the main FluxGit desktop application with a graphical user interface.
    Tauri,
    /// The command-line interface tool (`but`).
    ///
    /// This is the CLI tool for interacting with FluxGit from the terminal.
    Cli,
}
...
/// Checks if a newer version of the specified FluxGit application is available.
///
/// Performs a synchronous HTTP request to the FluxGit update server with a 30-second timeout.
...
pub fn check_status_with_url(
    ...
    headers.insert(USER_AGENT, HeaderValue::from_static("FluxGit"));
...
fn install() -> Option<String> {
    Some(format!(
        ...
        <sha2::Sha256 as sha2::Digest>::digest(
            format!("{}{}", machine_uid::get().ok()?, "fluxgit").as_bytes()
        )
    ))
}
