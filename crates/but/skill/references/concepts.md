# FluxGit CLI Key Concepts

Deep dive into FluxGit's conceptual model and philosophy.
...
### FluxGit: Parallel Stacks

```
workspace (fluxgit/workspace)
...
3. **The `fluxgit/workspace` branch**: A merge commit containing all applied stacks. Don't interact with it directly - use `but` commands.
...
**Dependency tracking:** FluxGit automatically tracks which changes depend on which commits. You can't stage dependent changes to the wrong branch.
...
FluxGit has MULTIPLE staging areas (one per branch):
...
FluxGit tracks dependencies between changes automatically.
...
3. If you try to move the change, FluxGit prevents invalid operations
...
Every operation in FluxGit is recorded in the oplog (operation log).
...
Think of it as "git reflog" but for all FluxGit operations, not just branch movements.
...
- Other FluxGit operations are limited
...
**Don't use in a FluxGit workspace:**
...
- `git rebase` - Conflicts with FluxGit's management
