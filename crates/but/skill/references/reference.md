...
Applied branches are merged into `fluxgit/workspace` and visible in working directory.
...
FluxGit has multiple staging areas - one per stack.
...
- `but absorb` - Let FluxGit auto-detect the target; smart matching based on dependencies
...
Initialize FluxGit in current git repository.
...
Converts regular git repo to use FluxGit workspace model. Use `--init` in non-interactive environments (CI/CD) to ensure a git repository exists before setup.
...
Open FluxGit GUI for current project.
...
Full documentation: <https://docs.fluxgit.com/cli-overview>
