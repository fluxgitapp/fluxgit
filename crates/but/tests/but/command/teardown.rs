...
fn single_branch_simple_teardown() -> anyhow::Result<()> {
    ...
    * edd3eb7 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    * 9477ae7 (A) add A
...
✓ Successfully exited FluxGit mode!

You are now on branch: A

To return to FluxGit mode, run:
  but setup
...
fn multiple_branches_preserves_state() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(env.git_log()?, @r"
    *   c128bce (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    |\  
    | * 9477ae7 (A) add A
    * | d3e2ba3 (B) add B
    |/  
...
/// Test 3: User has committed on top of fluxgit/workspace
...
/// Test 5: User has committed twice on top of fluxgit/workspace
...
fn two_dangling_commits_different_branches() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(env.git_log()?, @r"
    * fc13bfb (HEAD -> fluxgit/workspace) add FileForB
    * 091c8f9 add FileForA
    *   c128bce FluxGit Workspace Commit
    |\  
    | * 9477ae7 (A) add A
    * | d3e2ba3 (B) add B
    |/  
...
Attempting to fix workspace stacks...
→ Checking for dangling commits...
→ Resetting fluxgit/workspace to c128bce
  ✓ fluxgit/workspace reset to c128bce
