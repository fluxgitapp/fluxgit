...
fn first_run_shows_metrics_message() -> anyhow::Result<()> {
    ...
    let settings_path = env.app_data_dir().join("fluxgit/settings.json");
...
FluxGit uses metrics to help us know what is useful and improve it. Configure with `but config metrics`.
