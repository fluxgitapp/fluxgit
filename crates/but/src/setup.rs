...
/// the FluxGit project for that repository, and optionally triggers a background sync if
...
///     - `WorkspaceCheck::Enabled` - Check for non-workspace commits on fluxgit/workspace
...
pub fn init_ctx(
    ...
    // Check if we're on fluxgit/workspace with non-workspace commits on top
    ...
    let (ctx, fetch_interval_minutes, last_fetch) = {
        ...
        #[cfg(feature = "legacy")]
        {
            ...
            let project = match LegacyProject::find_by_worktree_dir(workdir) {
                Ok(project) => project,
                Err(_) => {
                    let message = format!("No FluxGit project found at {}", workdir.display());
        ...
        #[cfg(not(feature = "legacy"))]
        {
            ...
            //       i.e. once it was migrated out of `projects.json` into `.git/fluxgit/…`
            let fetch_interval_disabled = 0;
            (ctx, fetch_interval_disabled, None::<std::time::SystemTime>)
        }
    };

    // If this is the first time running FluxGit, show a metrics info message and update onboarding status
...
fn prompt_for_setup(out: &mut OutputChannel, message: &str) -> SetupPromptResult {
    ...
    _ = writeln!(
        progress,
        "The current project is not configured to be managed by FluxGit.\n"
    );
    writeln!(progress, "{}\n", message.red()).ok();
    ...
    if user_declined_in_interactive_mode {
        ...
        _ = writeln!(
            progress,
            "{}",
            "Please run `but setup` to switch to FluxGit management.\n".yellow()
        );
    }
...
fn check_workspace_commits_before_init(
    ...
    // Only check if we're on fluxgit/workspace
    if head_name != "fluxgit/workspace" {
        return Ok(());
    }

    // Check the HEAD commit message
    let mut workspace_ref = repo.find_reference("refs/heads/fluxgit/workspace")?;
    ...
    if !first_line.starts_with("FluxGit Workspace Commit") {
        if let Some(writer) = out.for_human() {
            writeln!(writer)?;
            writeln!(
                writer,
                "{}",
                "⚠ Detected commits on top of fluxgit/workspace"
                    .yellow()
                    .bold()
            )?;
            writeln!(writer)?;
            writeln!(
                writer,
                "{}",
                "FluxGit detected that you have committed directly on the\nfluxgit/workspace branch. To preserve your work and\nfix up things, please run `but teardown`.".dimmed()
            )?;
            writeln!(writer)?;
        }
        ...
        anyhow::bail!(
            "FluxGit mode exit required: please run `but teardown` to preserve your work."
        );
    }
