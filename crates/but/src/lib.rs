...
//! * `*legacy/`
//!     - code that depends on `fluxgit-*` crates or `but-*` crates with `legacy` feature enabled.
//!
//! ### Testing
//!
//! #### Legacy builds
...
pub async fn handle_args(args: impl Iterator<Item = OsString>) -> Result<()> {
    ...
    let namespace = option_env!("IDENTIFIER").unwrap_or("com.fluxgit.app");
    ...
    match cmd {
        ...
        Subcommands::Setup { init } => {
            let repo =
                ...
                {
                    fluxgit_project::AddProjectOutcome::Added(project)
                    | fluxgit_project::AddProjectOutcome::AlreadyExists(project) => {
            ...
            command::legacy::setup::repo(&mut ctx, &args.current_dir, out, guard.write_permission())
                .context("Failed to set up FluxGit project.")
                .emit_metrics(metrics_ctx)
        }
        #[cfg(feature = "legacy")]
        Subcommands::Teardown => {
            ...
            command::legacy::teardown::teardown(&mut ctx, out)
                .context("Failed to teardown FluxGit project.")
                .emit_metrics(metrics_ctx)
        }
        #[cfg(feature = "legacy")]
