...
pub enum Subcommands {
    /// Check if a new version of the FluxGit CLI is available
    Check,
    ...
    /// Install or update the FluxGit desktop application.
    ///
    /// By default, auto-detects your current channel (release/nightly) and installs the latest
    /// version for that channel.
    ///
    /// macOS: Installs the full FluxGit desktop application. The CLI (but) is included with the
    /// app and will also be updated.
    ///
    /// Linux: Installs and updates only the CLI itself.
    ///
    /// Note: For other platforms and install forms, see <https://fluxgit.com/downloads>
    #[cfg(unix)]
    Install {
