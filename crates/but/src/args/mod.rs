/// Command-line argument parsing for FluxGit CLI.
...
pub struct Args {
    ...
    pub trace: u8,
    /// Run as if fluxgit-cli was started in PATH instead of the current working directory.
...
pub enum Subcommands {
    ...
    /// Sets up a FluxGit project from a git repository in the current directory.
    ///
    /// This command will:
    /// - Add the repository to the global FluxGit project registry
    /// - Switch to the fluxgit/workspace branch (if not already on it)
    /// - Set up a default target branch (the remote's HEAD)
    /// - Add a gb-local remote if no push remote exists
    ///
    /// If you have an existing Git repository and want to start using FluxGit
    /// with it, you can run this command to set up the necessary configuration
    /// and data structures.
    ///
    /// ## Examples
    ///
    /// Initialize a new git repository and set up FluxGit:
    ///
    /// ```text
    /// but setup --init
    /// ```
    ...
    Setup {
        ...
        /// where you want to ensure a git repository exists before setting up FluxGit.
        #[clap(long)]
        #[clap(verbatim_doc_comment)]
        init: bool,
    },

    /// Exit FluxGit mode and return to normal Git workflow.
    ///
    /// This command:
    /// - Creates an oplog snapshot of the current state
    /// - Finds the first active branch and checks it out
    /// - Cherry-picks any dangling commits from fluxgit/workspace
    /// - Provides instructions on how to return to FluxGit mode
    ///
    /// This is useful when you want to temporarily or permanently leave FluxGit
    ...
    ///
    /// FluxGit worktrees allow you to have multiple working directories
    ...
    ///
    /// If you mark a branch, new unstaged changes that FluxGit sees when
    ...
    /// Open the FluxGit GUI for the current project.
    ///
    /// Running `but gui` will launch the FluxGit graphical user interface
    /// in the current directory's FluxGit project.
    ///
    ...
    /// automatic FluxGit commits after code generation or edits.
    ///
    ...
    ///
    /// See: <https://docs.fluxgit.com/features/ai-integration/claude-code-hooks>
    ///
    #[cfg(feature = "legacy")]
    #[clap(hide = true)]
    #[clap(verbatim_doc_comment)]
    ...
    ///
    /// See: <https://docs.fluxgit.com/features/ai-integration/cursor-hooks>
    ///
    #[cfg(feature = "legacy")]
    #[clap(hide = true)]
    #[clap(verbatim_doc_comment)]
    ...
    /// INTERNAL: FluxGit Actions are automated tasks (like macros) that can be performed on a repository.
    #[cfg(feature = "legacy")]
    #[clap(hide = true)]
    #[clap(verbatim_doc_comment)]
    ...
    /// Manage FluxGit CLI and app updates.
    ///
    ...
    /// View and manage FluxGit configuration.
    ///
    ...
    /// Manage Claude AI skills for FluxGit.
    ///
    /// Skills provide enhanced AI capabilities for working with FluxGit through
    /// Claude Code and other AI assistants.
    ///
    /// Use `but skill install` to install the FluxGit skill files. By default,
