...
pub(crate) fn teardown(ctx: &mut Context, out: &mut OutputChannel) -> anyhow::Result<()> {
    // Check that we're on fluxgit/workspace
    let head_name = {
    ...
    if !head_name.starts_with(b"fluxgit/") {
        if let Some(out) = out.for_human() {
            writeln!(
                out,
                "{}",
                format!("Not currently on fluxgit/workspace branch (on: {head_name}).").red()
            )?;
            writeln!(out)?;
            writeln!(
                out,
                "{}",
                "Teardown can only be run while on the fluxgit/workspace branch.".dimmed()
            )?;
        }
        anyhow::bail!("Not on fluxgit/workspace branch");
    }

    if let Some(out) = out.for_human() {
        writeln!(out, "{}", "Exiting FluxGit mode...".cyan().bold())?;
        writeln!(out)?;
    }
    ...
    let snapshot = but_api::legacy::oplog::create_snapshot(
        ctx,
        Some("Teardown: exiting FluxGit mode".to_string()),
    )?;
    ...
    if let Ok(git2_repo) = ctx.git2_repo.get()
        && let Err(e) = fluxgit_repo::managed_hooks::uninstall_managed_hooks(&git2_repo)
    ...
    if let Some(out) = out.for_human() {
        writeln!(
            out,
            "{}",
            "✓ Successfully exited FluxGit mode!".green().bold()
        )?;
        writeln!(out)?;
        writeln!(
            out,
        ...
        writeln!(out)?;
        writeln!(out, "{}", "To return to FluxGit mode, run:".blue())?;
...
// of fluxgit/workspace. Try to fix that.
fn try_stack_fixes(ctx: &mut Context, out: &mut OutputChannel) -> anyhow::Result<()> {
    ...
    // check if fluxgit/workspace is pointing at a commit that does not start with "FluxGit Workspace Commit"
    let repo = ctx.repo.get()?;

    // Look for dangling commits on fluxgit/workspace
    if let Some(out) = out.for_human() {
    ...
    if dangling_commits.is_empty() {
        ...
        if let Some(out) = out.for_human() {
            writeln!(
                ...
                "{}",
                format!(
                    "→ Resetting fluxgit/workspace to {}",
        ...
        repo.reference(
            ...
            target_commit,
            PreviousValue::Any,
            "soft resetting to FluxGit workspace",
        )?;
        if let Some(out) = out.for_human() {
            writeln!(
                out,
