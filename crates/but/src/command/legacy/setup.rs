...
/// Display a colorful splash screen with FluxGit branding and helpful commands
...
"#
    ...
    writeln!(
        out,
        "{}",
        "The command-line interface for FluxGit".dimmed()
    )?;
    writeln!(out)?;
    ...
    writeln!(
        out,
        "{}",
        "Learn more at https://docs.fluxgit.com/cli-overview".dimmed()
    )?;
    writeln!(out)?;
...
// setup a fluxgit project for the repository at `repo_path`
pub(crate) fn repo(
    ctx: &mut Context,
    repo_path: &Path,
    out: &mut OutputChannel,
    ...
    // find or setup the fluxgit project
    if let Some(out) = out.for_human() {
        writeln!(out, "{}", "Setting up FluxGit project...".cyan())?;
        writeln!(out)?;
        writeln!(
            out,
            "{}",
            "→ Adding repository to FluxGit project registry"
                .to_string()
                .dimmed()
        )?;
    }
    ...
    let project_status = match outcome {
        fluxgit_project::AddProjectOutcome::Added(_) => {
            if let Some(out) = out.for_human() {
                writeln!(
                    out,
                    "  {}",
        ...
        fluxgit_project::AddProjectOutcome::AlreadyExists(_) => {
            if let Some(out) = out.for_human() {
                writeln!(
                    out,
                    "  {}",
        ...
        fluxgit_project::AddProjectOutcome::PathNotFound => Err(anyhow::anyhow!(
            "The path {} does not exist",
            repo_path.display()
        )),
        fluxgit_project::AddProjectOutcome::NotADirectory => Err(anyhow::anyhow!(
            "The path {} is not a directory",
            repo_path.display()
        )),
        fluxgit_project::AddProjectOutcome::BareRepository => Err(anyhow::anyhow!(
            "The repository at {} is bare. FluxGit requires a non-bare repository.",
            repo_path.display()
        )),
        fluxgit_project::AddProjectOutcome::NonMainWorktree => Err(anyhow::anyhow!(
            "The repository at {} is a non-main worktree. FluxGit requires the main worktree.",
            repo_path.display()
        )),
        fluxgit_project::AddProjectOutcome::NoWorkdir => Err(anyhow::anyhow!(
            "The repository at {} has no working directory. FluxGit requires a working directory.",
            repo_path.display()
        )),
        fluxgit_project::AddProjectOutcome::NoDotGitDirectory => Err(anyhow::anyhow!(
            "The repository at {} has no .git directory. FluxGit requires a .git directory.",
            repo_path.display()
        )),
        fluxgit_project::AddProjectOutcome::NotAGitRepository(_) => Err(anyhow::anyhow!(
    ...
    if (matches!(outcome, fluxgit_project::AddProjectOutcome::Added(_))
        || matches!(
            outcome,
            fluxgit_project::AddProjectOutcome::AlreadyExists(_)
        ))
        && target.is_none()
    {
        ...
        if let Some(out) = out.for_human() {
            ...
            writeln!(
                out,
                "{}",
                "FluxGit project setup complete!".green().bold()
            )?;
        ...
        if let Some(out) = out.for_human() {
            ...
            writeln!(
                out,
                "{}",
                "FluxGit project is already set up!".green().bold()
            )?;
            if let Some(target) = target {
                writeln!(
                    out,
    ...
    // switch to fluxgit/workspace if not already there
    if !head_name.starts_with(b"fluxgit/") {
    ...
    if let Ok(git2_repo) = git2::Repository::open(repo_path)
        && let Err(e) = fluxgit_repo::managed_hooks::install_managed_hooks(&git2_repo)
        && let Some(out) = out.for_human()
    {
        writeln!(
            out,
            "  {}",
            format!("Warning: Failed to install FluxGit managed hooks: {e}").yellow()
        )?;
    }

    // if we switched - tell the user what this is all about
    if pre_head_name != "fluxgit/workspace"
        && let Some(out) = out.for_human()
    {
        writeln!(
            out,
            ...
            format!(
                r#"
Setting up your project for FluxGit tooling. Some things to note:

- Switching you to a special `fluxgit/workspace` branch to enable parallel branches
...
More info: https://docs.fluxgit.com/workspace-branch
"#
            )
            .yellow()
        )?;
...
/// Checks if a FluxGit project is set up for the repository at `repo_path`.
/// If so, returns true
/// Otherwise, returns an error with a message indicating what is not setup.
/// It will check:
/// - if the repository exists
/// - if the project is registered in FluxGit
/// - if there is a remote
/// - if there is a default target branch set
/// - if we're on a fluxgit/* branch
pub fn check_project_setup(ctx: &Context, perm: &RepoShared) -> anyhow::Result<bool> {
    ...
    // check if we're on a fluxgit/* branch
    let head = repo.head()?;
    let head_name = head
        .referent_name()
        .map(|n| n.shorten().to_owned())
        .unwrap_or_default();
    if !head_name.starts_with(b"fluxgit/") {
        anyhow::bail!("Not currently on a fluxgit/* branch.");
    }

    // When on fluxgit/edit, the project was already set up when entering edit mode.
    // The workspace graph built from fluxgit/edit doesn't expose the target ref or
    // remote configuration, but both are still configured in virtual_branches.toml
    // and will be accessible when returning to fluxgit/workspace.
    if head_name == b"fluxgit/edit" {
        return Ok(true);
    }
    ...
    repo.edit_reference(gix::refs::transaction::RefEdit {
        change: gix::refs::transaction::Change::Update {
            log: gix::refs::transaction::LogChange {
                ...
                message: "FluxGit local remote HEAD".into(),
            },
