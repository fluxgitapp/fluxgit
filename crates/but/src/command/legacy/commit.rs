...
pub(crate) fn commit(
    ...
    let final_commit_message = if !no_hooks {
        ...
        match hook_result {
            fluxgit_repo::hooks::MessageHookResult::Success => {
                // Hook passed without modification
                commit_message
            }
            fluxgit_repo::hooks::MessageHookResult::Message(message_data) => {
                // Hook passed and modified the message, use the new message
                message_data.message
            }
            fluxgit_repo::hooks::MessageHookResult::NotConfigured => {
                // No hook configured
                commit_message
            }
            fluxgit_repo::hooks::MessageHookResult::Failure(error_data) => {
                bail!(
