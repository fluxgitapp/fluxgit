...
use fluxgit_branch_actions::upstream_integration::{
...
async fn handle_pull(ctx: &Context, out: &mut OutputChannel) -> anyhow::Result<()> {
    ...
    if let Some((resolutions, statuses)) = resolutions {
        ...
        let mut branch_info_map: HashMap<fluxgit_stack::StackId, (String, String)> =
            HashMap::new();
        let mut resolution_map: HashMap<fluxgit_stack::StackId, ResolutionApproach> =
            HashMap::new();
