...
fn generate_branch_summary(
    ...
    // Get OpenAI provider (tries FluxGit proxied, own key, then env var)
    let llm = LLMProvider::from_git_config(git_config).ok_or_else(|| {
        anyhow::anyhow!(
            "No AI credentials found. Configure in FluxGit settings or set OPENAI_API_KEY environment variable."
        )
    })?;
