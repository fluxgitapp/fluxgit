...
pub fn apply(ctx: &mut Context, branch_name: &str, out: &mut OutputChannel) -> anyhow::Result<()> {
    ...
    let reference = if let Some(r) = repo.try_find_reference(branch_name)? {
        ...
        let ref_name = fluxgit_reference::Refname::from_str(&r.name().to_string())?;
        let remote_ref_name = r
            .remote_ref_name(gix::remote::Direction::Push)
            .transpose()?
            .as_deref()
            .and_then(|ref_name| {
                fluxgit_reference::RemoteRefname::from_str(&ref_name.to_string()).ok()
            });
        ...
        let ref_name =
            fluxgit_reference::Refname::from_str(&format!("refs/remotes/{remote}/{name}"))?;
        let r = r.detach();
        drop(repo);
        but_api::legacy::virtual_branches::create_virtual_branch_from_branch(
            ctx,
        ...
        match chosen {
            PartialMatch::Local { branch, .. } => {
                ...
                let remote_ref_name = r
                    ...
                    .as_deref()
                    .and_then(|rn| {
                        fluxgit_reference::RemoteRefname::from_str(&rn.to_string()).ok()
                    });
                let r = r.detach();
                drop(repo);
                but_api::legacy::virtual_branches::create_virtual_branch_from_branch(
            ...
            PartialMatch::Remote { remote, branch, .. } => {
                ...
                let ref_name = fluxgit_reference::Refname::from_str(&format!(
