...
/// This represents the status of the FluxGit "workspace".
...
/// Represents a branch in the FluxGit workspace
#[derive(Clone, Debug, Serialize)]
#[serde(rename_all = "camelCase")]
...
/// A commit that is in the FluxGit workspace
#[derive(Clone, Debug, Serialize)]
#[serde(rename_all = "camelCase")]
...
fn convert_branch_to_json(
    ...
    branch_merge_statuses: &BTreeMap<
        String,
        fluxgit_branch_actions::upstream_integration::BranchStatus,
    >,
    ...
    let merge_status = segment.branch_name().and_then(|name| {
        branch_merge_statuses
            ...
            .map(|status| match status {
                fluxgit_branch_actions::upstream_integration::BranchStatus::SaflyUpdatable => {
                    MergeStatus::Clean
                }
                fluxgit_branch_actions::upstream_integration::BranchStatus::Integrated => {
                    MergeStatus::Integrated
                }
                fluxgit_branch_actions::upstream_integration::BranchStatus::Conflicted {
                    rebasable,
                } => MergeStatus::Conflicted {
                    rebasable: *rebasable,
                },
                fluxgit_branch_actions::upstream_integration::BranchStatus::Empty => {
                    MergeStatus::Empty
                }
            })
    });
...
pub(super) fn build_workspace_status_json(
    ...
    repo: &gix::Repository,
    id_map: &crate::IdMap,
    base_branch: Option<&fluxgit_branch_actions::BaseBranch>,
    show_upstream: bool,
