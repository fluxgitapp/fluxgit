...
use fluxgit_branch_actions::upstream_integration::BranchStatus as UpstreamBranchStatus;
...
type StackEntry = (Option<fluxgit_stack::StackId>, StackDetail);
...
pub(crate) async fn worktree(
    ...
    let mode = fluxgit_operating_modes::operating_mode(ctx);
    if let fluxgit_operating_modes::OperatingMode::Edit(_metadata) = mode {
    ...
    let (common_merge_base_data, upstream_state, last_fetched_ms, base_branch) = {
        let stack = fluxgit_stack::VirtualBranchesHandle::new(ctx.project_data_dir());
    ...
    let not_on_workspace = matches!(
        mode,
        fluxgit_operating_modes::OperatingMode::OutsideWorkspace(_)
    );
...
⚠️    More info: https://github.com/fluxgitapp/fluxgit/issues/11866                     ⚠️
"#,
        )?;
    }
    ...
    if hint {
        ...
        if not_on_workspace {
            writeln!(
                out,
                "{}",
                "Hint: run `but setup` to switch back to FluxGit managed mode.".dimmed()
            )?;
...
async fn compute_branch_merge_statuses(
    ...
    use fluxgit_branch_actions::upstream_integration::StackStatuses;
