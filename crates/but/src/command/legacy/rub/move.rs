...
use fluxgit_branch_actions::reorder::commits_order;
...
fn move_to_branch(
    ...
    if source_stack_id == target_stack_id {
        ...
        fluxgit_branch_actions::reorder_stack(ctx, source_stack_id, stack_order)?;
        ...
        if let Some(illegal_move) = fluxgit_branch_actions::move_commit(
            ctx,
            target_stack_id,
            source_oid.to_git2(),
            source_stack_id,
            ...
            if let Some(out) = out.for_human() {
                match &illegal_move {
                    fluxgit_branch_actions::MoveCommitIllegalAction::DependsOnCommits(deps) => {
                        writeln!(
                            out,
                    ...
                    fluxgit_branch_actions::MoveCommitIllegalAction::HasDependentChanges(deps) => {
                        writeln!(
                            out,
                    ...
                    fluxgit_branch_actions::MoveCommitIllegalAction::HasDependentUncommittedChanges => {
                        writeln!(
                            out,
