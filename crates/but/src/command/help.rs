...
pub fn print_grouped(out: &mut dyn std::fmt::Write) -> std::fmt::Result {
    ...
    writeln!(out, "{}", "The FluxGit CLI change control system".red())?;
    writeln!(out)?;
    ...
    writeln!(
        out,
        "The FluxGit CLI can be used to do nearly anything the desktop client can do (and more)."
    )?;
    writeln!(
        out,
    ...
    writeln!(
        out,
        "commands (blame, log, etc) can also be used, as FluxGit is fully Git compatible."
    )?;
    writeln!(out)?;
    writeln!(
        out,
        "Checkout the full docs here: https://docs.fluxgit.com/cli-overview"
    )?;
    writeln!(out)?;
    ...
    writeln!(
        out,
        "To use the FluxGit CLI with coding agents (Claude Code hooks, Cursor hooks, MCP), see:"
    )?;
    writeln!(
        out,
        "  https://docs.fluxgit.com/features/ai-integration/ai-overview"
    )?;
    writeln!(out)?;
