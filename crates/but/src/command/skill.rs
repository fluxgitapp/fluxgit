...
/// Validate that a SKILL.md file is actually a FluxGit skill
fn is_fluxgit_skill(skill_md_path: &std::path::Path) -> bool {
    if let Ok(content) = std::fs::read_to_string(skill_md_path) {
        // Check for FluxGit-specific markers with proper context
        ...
        let has_fluxgit_header = content.lines().any(|line| {
            line.contains("# FluxGit CLI Skill") || line.contains("FluxGit CLI (`but` command)")
        });

        has_frontmatter_name || has_fluxgit_header
    } else {
        false
    }
}
...
/// Find all FluxGit skill installations.
///
...
fn find_all_installations(
    ...
    for (base_dir, scope) in base_dirs {
        for format in SKILL_FORMATS {
            ...
            if skill_md_path.exists() && is_fluxgit_skill(&skill_md_path) {
...
fn print_human_check_output(
    ...
    if result.skills.is_empty() {
        writeln!(writer, "No FluxGit skill installations found.")?;
        writeln!(writer)?;
...
fn install_skill(
    ...
    writeln!(
        progress,
        "{} FluxGit skill installed successfully!",
        "✓".green().bold()
    )?;
    writeln!(progress)?;
    writeln!(
...
mod tests {
    ...
    // integration tests and manual testing. The core logic (is_fluxgit_skill validation
    // and scope prioritization) is tested separately.

    #[test]
    fn is_fluxgit_skill_validates_correct_skill() {
        use std::fs;
        ...
        // Test with valid FluxGit skill content (frontmatter)
        fs::write(
            &skill_path,
            "---\nname: but\nversion: 1.0.0\n---\n# Content",
        )
        .unwrap();
        assert!(
            is_fluxgit_skill(&skill_path),
            "Should recognize valid FluxGit skill with frontmatter"
        );

        // Test with valid FluxGit skill content (header)
        fs::write(&skill_path, "# FluxGit CLI Skill\n\nContent here").unwrap();
        assert!(
            is_fluxgit_skill(&skill_path),
            "Should recognize valid FluxGit skill with header"
        );

        // Test with invalid content that contains the strings but not as exact matches
        fs::write(
            &skill_path,
            "I was reading about the FluxGit CLI and the name: but that's not right",
        )
        .unwrap();
        assert!(
            !is_fluxgit_skill(&skill_path),
        ...
        assert!(
            !is_fluxgit_skill(&skill_path),
            "Should reject non-FluxGit content"
        );

        // Test with nonexistent file
        let nonexistent = temp_dir.path().join("nonexistent.md");
        assert!(
            !is_fluxgit_skill(&nonexistent),
    ...
    fn find_all_installations_discovers_skills_in_temp_dir() {
        ...
        let claude_skill_dir = temp_dir.path().join(".claude/skills/fluxgit");
        ...
        let cursor_skill_dir = temp_dir.path().join(".cursor/skills/fluxgit");
        ...
        // Create a non-FluxGit skill (should be ignored)
        let other_skill_dir = temp_dir.path().join(".opencode/skills/fluxgit");
        ...
        // Verify is_fluxgit_skill correctly identifies our test files
        assert!(is_fluxgit_skill(&claude_skill_dir.join("SKILL.md")));
        assert!(is_fluxgit_skill(&cursor_skill_dir.join("SKILL.md")));
        assert!(!is_fluxgit_skill(&other_skill_dir.join("SKILL.md")));
