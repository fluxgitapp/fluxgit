...
impl<'repo> WorkspaceCommit<'repo> {
    const FLUXGIT_WORKSPACE_COMMIT_TITLE: &'static str = "FluxGit Workspace Commit";
    ...
    pub fn new_from_stacks(
        ...
        let mut message = Self::FLUXGIT_WORKSPACE_COMMIT_TITLE.to_string();
        message.push_str("\n\n");
        if !stacks.is_empty() {
            message
        ...
        message
            .push_str("https://docs.fluxgit.com/features/branch-management/integration-branch\n");
...
fn commit_signature(time: gix::date::Time) -> gix::actor::Signature {
    gix::actor::Signature {
        name: "FluxGit".into(),
        email: "fluxgit@fluxgit.com".into(),
        time,
    }
}
...
/// Query
impl WorkspaceCommit<'_> {
    /// Return `true` if this commit is managed by FluxGit.
