...
/// This code is a fork of [`fluxgit_branch_actions::author`] to avoid depending on the `fluxgit_branch_actions` crate.
mod author;
...
/// Commit that is a part of a [`StackBranch`](fluxgit_stack::StackBranch) and, as such, containing state derived in relation to the specific branch.
...
pub struct Commit {
    ...
    /// The Conflicted state of a commit is a FluxGit concept.
    /// FluxGit will perform rebasing/reordering etc without interruptions and flag commits as conflicted if needed.
...
/// Unlike the `Commit` struct, there is no knowledge of FluxGit concepts like conflicted state etc.
#[derive(Clone, Serialize)]
#[cfg_attr(feature = "export-ts", derive(ts_rs::TS))]
#[cfg_attr(feature = "export-schema", derive(schemars::JsonSchema))]
#[serde(rename_all = "camelCase")]
...
pub struct BranchDetails {
    ...
    /// A unique identifier for the FluxGit review associated with the branch, if any.
