...
pub struct Commit {
    ...
    /// Whether the commit is in a conflicted state, a FluxGit concept.
    /// FluxGit will perform rebasing/reordering etc. without interruptions and flag commits as conflicted if needed.
    /// Conflicts are resolved via the Edit Mode mechanism.
    ///
    /// Note that even though FluxGit won't push branches with conflicts, the user can still push such branches at will.
    pub has_conflicts: bool,
    /// The FluxGit assigned change-id that we hold on to for convenience to avoid duplicate decoding of commits
...
pub(crate) mod function {
    ...
    /// Like [`workspace_data_of_workspace_branch()`], but it will try the name of the default FluxGit workspace branch.
