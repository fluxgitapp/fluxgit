...
pub(crate) mod function {
    ...
    use but_core::{
        ObjectStorageExt, RefMetadata, RepositoryExt, extract_remote_name_and_short_name,
        ref_metadata,
        ref_metadata::{
            StackId, StackKind,
            ...
            let ws = ws
                ...
                .redo_traversal_with_overlay(
                    ...
                    Overlay::default().with_entrypoint(
                        commit_to_checkout.id,
                        ws.ref_name().map(|rn| rn.to_owned()),
                    ),
                )?
                .into_workspace()?;
            ...
            WorkspaceKind::AdHoc => {
                ...
                let next_ws_ref_name = match workspace_reference_naming {
                    WorkspaceReferenceNaming::Default => {
                        gix::refs::FullName::try_from("refs/heads/fluxgit/workspace")
    ...
    fn persist_metadata_and_gitconfig<T: RefMetadata>(
        ...
        if let Some((config, (ref_to_create, remote_tracking_ref, ref_target_id))) = config_and_ref
        {
            ...
            repo.reference(
                ...
                format!("FluxGit creates local tracking for {remote_tracking_ref}"),
            )?;
        }
        Ok(())
    }
    ...
    fn set_head_to_reference(
        ...
        let edits = match new_ref {
            None => {
                let head_message = "FluxGit checkout workspace during apply-branch".into();
                vec![RefEdit {
            ...
            Some(new_ref) => {
                ...
                let head_message = "FluxGit switch to workspace during apply-branch".into();
                vec![
                    RefEdit {
                        change: Change::Update {
                            log: LogChange {
                    ...
                    RefEdit {
                        change: Change::Update {
                            log: LogChange {
                                mode: RefLog::AndReference,
                                force_create_reflog: false,
                                message: "created by FluxGit during apply-branch".into(),
                            },
