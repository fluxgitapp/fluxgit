...
pub fn create_commit_and_update_refs(
    ...
    if let Some(commit_in_graph) = commit_to_find {
        ...
        let checked_out_and_fluxgit_refs =
            checked_out_ref
                .into_iter()
                // TODO: remove this as `refs/fluxgit/` won't contain relevant refs anymore.
                .chain({
                    platform_storage_2 = repo.references()?;
                    platform_storage_2
                        .prefixed("refs/fluxgit/")?
        ...
        for (commit_id, git_reference) in checked_out_and_fluxgit_refs {
            all_refs_by_id
                .entry(commit_id)
                .or_default()
                .push(git_reference);
            ...
            && all_refs_by_id.contains_key(&commit_in_graph)
        {
            ...
            let rebase = {
                ...
                if let Some(workspace_tip) = workspace_tip {
                    // Special Hack (https://github.com/fluxgitapp/fluxgit/pull/7976)
...
/// Like [`create_commit_and_update_refs()`], but integrates with an existing FluxGit `project`
