...
mod with_workspace {
    ...
    fn journey_no_ws_commit_no_target() -> anyhow::Result<()> {
        ...
        insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @"* 3183e43 (HEAD -> fluxgit/workspace, main) M1");
        ...
        insta::assert_snapshot!(graph_workspace(&ws), @"📕🏘️⚠️:0:fluxgit/workspace[🌳] <> ✓! on 3183e43");
        ...
        assert_eq!(
            err.to_string(),
            "workspace at refs/heads/fluxgit/workspace is missing a base",
    ...
    fn journey_no_ws_commit() -> anyhow::Result<()> {
        ...
        insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @"* 3183e43 (HEAD -> fluxgit/workspace, origin/main, main) M1");

        let graph = but_graph::Graph::from_head(&repo, &meta, Options::limited())?;
        let ws = graph.into_workspace()?;

        insta::assert_snapshot!(graph_workspace(&ws), @"📕🏘️⚠️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main on 3183e43");
        ...
        insta::assert_snapshot!(graph_workspace(&ws), @r"
        📕🏘️⚠️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main on 3183e43
        └── ≡📙:3:A on 3183e43 {41}
            └── 📙:3:A
        ");
        ...
        insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @"* 3183e43 (HEAD -> fluxgit/workspace, origin/main, main, below-B, above-A, B, A) M1");
    ...
    fn journey_single_branch_segment_anchor() -> anyhow::Result<()> {
        ...
        * 05240ea (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
        * 43f9472 (A) A2
        * 6fdab32 A1
        ...
        * 3183e43 M1
        ...
        insta::assert_snapshot!(graph_workspace(&ws), @r"
        📕🏘️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main on bce0c5e
        └── ≡:3:A on bce0c5e
            └── :3:A
                ├── ·43f9472 (🏘️)
                └── ·6fdab32 (🏘️)
    ...
    fn journey_single_branch_no_ws_commit_segment_anchor() -> anyhow::Result<()> {
        ...
        insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
        * c2878fb (HEAD -> fluxgit/workspace, A) A2
        * 49d4b34 A1
        * 3183e43 (origin/main, main) M1
        ...
        insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
        * c2878fb (HEAD -> fluxgit/workspace, above-A-commit, above-A, A) A2
    ...
    fn journey_multi_branch_commit_anchor() -> anyhow::Result<()> {
        ...
        insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
        *   eaf2834 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
        |\  
        | * 49d4b34 (A) A1
        * | f57c528 (B) B1
        |/  
    ...
    fn error1() -> anyhow::Result<()> {
        ...
        insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
        * bce0c5e (HEAD -> fluxgit/workspace, main) M2
        * 3183e43 (origin/main) M1
        ...
        insta::assert_snapshot!(graph_workspace(&ws), @"📕🏘️⚠️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main⇣1 on bce0c5e");

        let (ws_id, ws_ref_name) = id_at(&repo, "fluxgit/workspace");
    ...
    fn error2() -> anyhow::Result<()> {
        ...
        * 3183e43 (origin/main, main) M1
        ...
        let (ws_id, ws_ref_name) = id_at(&repo, "fluxgit/workspace");
        // Try to set fluxgit/workspace to a position in the workspace, but one below its current position
        ...
        for anchor in [
            ...
            assert_eq!(
                err.to_string(),
                "Reference 'fluxgit/workspace' cannot be created as segment at 49d4b34f36239228b64ee758be8f58849bac02d5",
        ...
        // Try to set fluxgit/workspace to the same position, which technically is in the workspace
        ...
        for anchor in [
            ...
            assert_eq!(
                err.to_string(),
                "Reference 'fluxgit/workspace' cannot be created as segment at c2878fb5dda8243a099a0353452d497d906bc6b5",
