...
fn operation_denied_on_improper_workspace() -> anyhow::Result<()> {
    ...
    * 0d01196 (HEAD -> fluxgit/workspace) O1
    * 4979833 FluxGit Workspace Commit
    * 3183e43 (main, B, A) M1
    ");
    let ws = graph.into_workspace()?;
    insta::assert_snapshot!(graph_workspace(&ws), @r"
    📕🏘️⚠️:0:fluxgit/workspace[🌳] <> ✓! on 3183e43
    └── ≡:2:anon: on 3183e43
        └── :2:anon:
            ├── ·0d01196 (🏘️)
            └── ·4979833 (🏘️)
...
-> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @"* e5d0542 (HEAD -> fluxgit/workspace, main, B, A) A");

    // We know a stack, but nothing is actually in the workspace.
    let ws = graph.into_workspace()?;
    insta::assert_snapshot!(graph_workspace(&ws), @"📕🏘️⚠️:0:fluxgit/workspace[🌳] <> ✓! on e5d0542");
    ...
    insta::assert_snapshot!(graph_workspace(&ws), @r"
    📕🏘️⚠️:0:fluxgit/workspace[🌳] <> ✓! on e5d0542
    └── ≡📙:2:B on e5d0542 {1}
        └── 📙:2:B
    ");
...
fn main_with_advanced_remote_tracking_branch() -> anyhow::Result<()> {
    ...
    * 3183e43 (HEAD -> main) M1
    ...
    meta.workspaces.push((
        "refs/heads/fluxgit/workspace".try_into()?,
    ...
    insta::assert_snapshot!(graph_workspace(&ws), @r"
    📕🏘️:0:fluxgit/workspace[🌳] <> ✓! on 3183e43
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    *   e00cf08 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    |\  
...
fn workspace_with_out_of_ws_ref_and_anon_stack() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * d03b217 (feature) F1
    | *   dd3b979 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    | |\  
...
-> anyhow::Result<()> {
    ...
    {
        let mut ws_md = meta.workspace("refs/heads/fluxgit/workspace".try_into().unwrap())?;
        assert!(ws_md.target_ref.is_some());
        ws_md.target_ref.take();
        meta.set_workspace(&ws_md)?;
        let ws_md = meta.workspace("refs/heads/fluxgit/workspace".try_into().unwrap())?;
        assert!(
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @"* e5d0542 (HEAD -> fluxgit/workspace, origin/main, main, B, A) A");
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * 6277161 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    * e5d0542 (origin/main, main, B, A) A
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * 452772e (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    |\
    * e5d0542 (origin/main, main, B, A) A
    ");
    ...
    let ws = out.workspace.into_owned();
    insta::assert_snapshot!(graph_workspace(&ws), @r"
    📕🏘️:0:fluxgit/workspace[🌳] <> ✓! on e5d0542
    ├── ≡📙:3:A on e5d0542 {41}
    │   └── 📙:3:A
    └── ≡📙:2:B on e5d0542 {42}
        └── 📙:2:B
...
-> anyhow::Result<()> {
    ...
    let ws = out.workspace.into_owned();
    insta::assert_snapshot!(graph_workspace(&ws), @r"
    📕🏘️⚠️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main on e5d0542
    └── ≡📙:3:A on e5d0542 {41}
        └── 📙:3:A
    ");
...
fn new_workspace_exists_elsewhere_and_to_be_applied_branch_exists_there() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(graph_workspace(&ws_graph.into_workspace()?), @"📕🏘️⚠️:0:fluxgit/workspace[🌳] <> ✓! on e5d0542");
    ...
    let ws = graph.into_workspace()?;
    // Note how the existing `fluxgit/workspace` disappears, which is expected here.
    insta::assert_snapshot!(graph_workspace(&ws), @"
    ⌂:1:B <> ✓!
    └── ≡:1:B {1}
        └── :1:B
...
fn apply_multiple_without_target_or_metadata_or_base() -> anyhow::Result<()> {
    ...
    * e31e6ca (origin/main, origin/HEAD) add init
    ...
    let ws = out.workspace.into_owned();
    insta::assert_snapshot!(graph_workspace(&ws), @r"
    📕🏘️:0:fluxgit/workspace[🌳] <> ✓! on e31e6ca
    ├── ≡📙:2:A on e31e6ca {41}
    │   └── 📙:2:A
    │       └── ·bf53300 (🏘️)
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    *   d87b903 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    |\  
    | * bf53300 (A) add A
    * | b1540e5 (main) M
    |/  
    ...
    * e31e6ca (origin/main, origin/HEAD) add init
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    *-.   7bcf528 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    |\ \  
...
fn apply_multiple_segments_of_stack_in_order_merge_if_needed() -> anyhow::Result<()> {
    ...
    * f1889e7 (A2) add A2
    * 7de99e1 (A1) add A1
    | * 6848743 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    | * 53ad0c2 (unrelated) add U1
    |/  
    * 3183e43 (origin/main, main) M1
    ");
...
fn detached_head_journey() -> anyhow::Result<()> {
    ...
    * 49d4b34 (A) A1
    | * f57c528 (B) B1
    |/  
    | * aaa195b (HEAD -> fluxgit/workspace, C) C1
    |/  
    * 3183e43 (main) M1
    ");
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * 49d4b34 (A) A1
    | *   fdec130 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    | |\  
    | | * f57c528 (B) B1
    | |/  
    |/|   
    ...
    * 3183e43 (main) M1
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    *-.   951ff29 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    |\ \  
    | | * f57c528 (B) B1
    | * | aaa195b (C) C1
    | |/  
...
fn apply_two_ambiguous_stacks_with_target_with_dependent_branch() -> anyhow::Result<()> {
    ...
    * 85efbe4 (HEAD -> main, origin/main) M
    ...
    let ws = out.workspace.into_owned();
    insta::assert_snapshot!(graph_workspace(&ws), @r"
    📕🏘️:0:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main on 85efbe4
    └── ≡📙:4:E on 85efbe4 {1}
        └── 📙:4:E
            └── ·7076dee (🏘️) ►D
    ");
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    *   78f3659 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    |\  
    | * f084d61 (C, B, A) A2
    |/  
    * 7076dee (E, D) A1
...
fn apply_two_ambiguous_stacks_with_target() -> anyhow::Result<()> {
    ...
    * 85efbe4 (HEAD -> main, origin/main) M
    ...
    ");
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * 773e030 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    * f084d61 (C, B, A) A2
    * 7076dee (E, D) A1
    * 85efbe4 (origin/main, main) M
    ");
    ...
    ");
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    * b390237 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    * f084d61 (C, B, A) A2
    * 7076dee (E, D) A1
    * 85efbe4 (origin/main, main) M
    ");
...
fn apply_with_conflicts_shows_exact_conflict_info() -> anyhow::Result<()> {
    ...
    * d3cce74 (clean-A) add A
    ...
    |/  
    * 85efbe4 (main, fluxgit/workspace) M
    ");

    assert!(git(&repo).args(["checkout", "main"]).status()?.success());
    assert!(
        git(&repo)
            .args(["branch", "-d", "fluxgit/workspace"])
            .status()?
            .success()
    );
    let mut ws = graph
    ...
    insta::assert_snapshot!(graph_workspace(&ws), @r"
    📕🏘️:0:fluxgit/workspace[🌳] <> ✓! on 85efbe4
    ...
    * 98519e9 add conflicting-F1
    | *-----.   e13e11a (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    |/|\ \ \ \  
    | | | | | * 34c4591 (clean-C) add C
    | |_|_|_|/  
    |/| | | |   
    ...
    * bf09eae (conflict-F1) add F1
    | * f2ce66d (conflict-F2) add F2
    |/  
    | *---.   c51f37c (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    |/|\ \ \  
    | | | | * 4bbb93c (conflict-hero) add conflicting-F2
    | | | | * 98519e9 add conflicting-F1
    | |_|_|/  
...
fn auto_checkout_of_enclosing_workspace_flat() -> anyhow::Result<()> {
    ...
    let out = but_workspace::branch::apply(
        r("refs/heads/fluxgit/workspace"),
        &ws,
        &repo,
        &mut meta,
        default_options(),
    ...
    Outcome {
        workspace_changed: false,
        workspace_ref_created: false,
        applied_branches: "[refs/heads/fluxgit/workspace]",
    }
    "#);
    ...
    .into_workspace()?;
    insta::assert_snapshot!(graph_workspace(&ws), @r"
    📕🏘️⚠️:1:fluxgit/workspace[🌳] <> ✓! on e5d0542
    ├── ≡👉📙:3:B on e5d0542 {2}
    │   └── 👉📙:3:B
    └── ≡📙:2:A on e5d0542 {1}
        └── 📙:2:A
...
fn auto_checkout_of_enclosing_workspace_with_commits() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @r"
    *   c49e4d8 (HEAD -> fluxgit/workspace) FluxGit Workspace Commit
    |\  
    | * 09d8e52 (A) A
    * | c813d8d (B) B
    |/  
    * 85efbe4 (origin/main, main) M
    ...
    // Apply the workspace ref itself, it's a no-op
    let ws_ref = r("refs/heads/fluxgit/workspace");
    ...
    let ws =
        but_graph::Graph::from_commit_traversal(b_id, b_ref.clone(), &meta, Default::default())?
            .into_workspace()?;
    insta::assert_snapshot!(graph_workspace(&ws), @r"
    📕🏘️:1:fluxgit/workspace[🌳] <> ✓refs/remotes/origin/main on 85efbe4
    ├── ≡👉📙:0:B on 85efbe4 {2}
    │   └── 👉📙:0:B
    │       └── ·c813d8d (🏘️)
    └── ≡📙:4:A on 85efbe4 {1}
    ...
    assert_eq!(
        err.to_string(),
        "Refusing to apply a reference that already is a workspace: 'fluxgit/workspace'",
