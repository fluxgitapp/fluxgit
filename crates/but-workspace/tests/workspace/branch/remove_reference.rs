...
fn no_errors_due_to_idempotency_in_empty_workspace() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @"* 3183e43 (HEAD -> fluxgit/workspace, main, B, A) M1");
    let ws = graph.into_workspace()?;
    // the workspace is empty.
    insta::assert_snapshot!(graph_workspace(&ws), @"📕🏘️⚠️:0:fluxgit/workspace[🌳] <> ✓! on 3183e43");

    for name in ["A", "B", "fluxgit/workspace", "main", "nonexisting"] {
        assert!(
...
fn journey_single_branch_no_ws_commit_no_target() -> anyhow::Result<()> {
    ...
    * c2878fb (HEAD -> fluxgit/workspace, A2, A) A2
    * 49d4b34 (A1) A1
...
fn journey_no_ws_commit_no_target() -> anyhow::Result<()> {
    ...
    insta::assert_snapshot!(desc, @"Single commit, no main remote/target, no ws commit, but ws-reference");
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @"* 3183e43 (HEAD -> fluxgit/workspace, main, E, D, C, B, A) M1");
    ...
    insta::assert_snapshot!(visualize_commit_graph_all(&repo)?, @"* 3183e43 (HEAD -> fluxgit/workspace, main, A) M1");
    let ws = ws.graph.into_workspace_of_redone_traversal(&repo, &meta)?;
