...
use fluxgit_branch_actions::BranchListingFilter;
...
pub mod stacks {
    ...
    fn create_stack_with_branch(
        ...
        if remote {
            ...
            let (stack_id, _, _) = fluxgit_branch_actions::create_virtual_branch_from_branch(
                ctx,
                &ref_name,
        ...
        let creation_request = fluxgit_branch::BranchCreateRequest {
    ...
    fn add_branch_to_stack(
        ...
        let creation_request = fluxgit_branch_actions::stack::CreateSeriesRequest {
            name: name.to_string(),
            target_patch: None,
            preceding_head: None,
        };

        fluxgit_branch_actions::stack::create_branch(ctx, stack_id, creation_request)?;
        let repo = ctx.repo.get()?;
        let meta = but_meta::VirtualBranchesTomlMetadata::from_path(
            ctx.project_data_dir().join("virtual_branches.toml"),
        )?;
    ...
    pub fn move_branch(
        ...
        let outcome = fluxgit_branch_actions::move_branch(
            &mut ctx,
            destination_stack
                .id
...
pub async fn watch(args: &super::Args, watch_mode: Option<&str>) -> anyhow::Result<()> {
    ...
    let _monitor = fluxgit_filemonitor::spawn(
        ProjectId::generate(),
        &workdir,
        tx,
...
pub fn operating_mode(args: &super::Args) -> anyhow::Result<()> {
    let ctx = Context::discover(&args.current_dir)?;
    debug_print(fluxgit_operating_modes::operating_mode(&ctx))
}
...
pub fn branch_list(current_dir: &Path) -> anyhow::Result<()> {
    let ctx = Context::discover(current_dir)?;
    debug_print(fluxgit_branch_actions::list_branches(
        &ctx,
