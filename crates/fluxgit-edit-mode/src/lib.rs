...
use fluxgit_branch_actions::update_workspace_commit;
use fluxgit_cherry_pick::{ConflictedTreeKey, RepositoryExt as _};
use fluxgit_commit::commit_ext::{CommitExt, CommitMessageBstr as _};
use fluxgit_operating_modes::{
...
use fluxgit_repo::{RepositoryExt as _, SignaturePurpose, signature};
use fluxgit_stack::VirtualBranchesHandle;
use fluxgit_workspace::branch_trees::{WorkspaceState, update_uncommitted_changes_with_tree};
use serde::Serialize;

pub mod commands;

const UNCOMMITTED_CHANGES_REF: &str = "refs/fluxgit/edit-uncommitted-changes";
...
/// Returns a commit to be the HEAD of `fluxgit/edit`
///
...
pub(crate) fn abort_and_return_to_workspace(
    ...
    // Checkout fluxgit workspace branch
...
pub(crate) fn save_and_return_to_workspace(ctx: &Context, perm: &mut RepoExclusive) -> Result<()> {
    ...
    // Switch branch to fluxgit/workspace
    repo.set_head(WORKSPACE_BRANCH_REF)
        .context("Failed to set head reference")?;
