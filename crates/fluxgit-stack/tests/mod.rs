...
use fluxgit_stack::{PatchReferenceUpdate, Stack, StackBranch, VirtualBranchesHandle};
...
fn add_head_with_archived_bottom_head() -> Result<()> {
    ...
    let updated_heads = fluxgit_stack::add_head(
        existing_heads,
        new_head.clone(),
        Some(head_2.clone()),
        patches,
...
fn command_ctx(name: &str) -> Result<(Context, TempDir)> {
    fluxgit_testsupport::writable::fixture("stacking.sh", name)
}
...
fn stack_target(branch: &str, sha: &str) -> Result<fluxgit_stack::Target> {
    let oid = git2::Oid::from_str(sha)?;
    if oid.is_zero() {
        bail!("test target oid must not be zero");
    }
    Ok(fluxgit_stack::Target {
        branch: RemoteRefname::new("origin", branch),
        remote_url: "https://example.invalid/repo".into(),
        sha: oid,
        push_remote_name: Some("origin".into()),
