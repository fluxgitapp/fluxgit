...
use git2::Commit;
use fluxgit_reference::{Refname, RemoteRefname, VirtualRefname, normalize_branch_name};
use fluxgit_repo::logging::{LogUntil, RepositoryExt as _};
...
pub struct Stack {
    ...
    /// Do **NOT** edit this directly, instead use the `Stack` trait in fluxgit_stack.
...
///
/// This operates via a list of PatchReferences (heads) that are an attribute of fluxgit_branch::Branch.
...
impl Stack {
    ...
    /// This is in fact just creating a new  FluxGit patch reference (head) and associates it with the stack.
    ...
    ///
    /// This operation mutates the fluxgit::Branch.heads list and updates the state in `virtual_branches.toml`
    pub fn add_series(
        &mut self,
        ctx: &Context,
