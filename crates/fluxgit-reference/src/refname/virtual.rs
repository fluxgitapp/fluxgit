...
pub struct Refname {
    ...
    /// Note that `Display` is implemented to put `refs/fluxgit/` in front of `branch`
...
impl fmt::Display for Refname {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "refs/fluxgit/{}", self.branch)
    }
}
...
impl FromStr for Refname {
    ...
    fn from_str(value: &str) -> Result<Self, Self::Err> {
        if !value.starts_with("refs/fluxgit/") {
            return Err(Error::NotLocal(value.to_string()));
        }

        if let Some(branch) = value.strip_prefix("refs/fluxgit/") {
            Ok(Self {
