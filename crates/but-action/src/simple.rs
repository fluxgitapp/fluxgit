...
use fluxgit_operating_modes::OperatingMode;
use fluxgit_oplog::{
    OplogExt,
...
/// This is a FluxGit automation which allows easy handling of uncommitted changes in a repository.
/// At a high level, it will:
///   - Checkout FluxGit's workspace branch if not already checked out
...
///
/// Avery time this automation is ran, FluxGit will also:
...
fn handle_changes_simple_inner(
    ...
    match fluxgit_operating_modes::operating_mode(ctx) {
        ...
        OperatingMode::OutsideWorkspace(_) => {
            ...
            fluxgit_branch_actions::set_base_branch(ctx, &default_target.branch, perm)?;
        }
    }
