...
CRATE_ROOT="$ROOT/crates/fluxgit-tauri"

# BINARIES
ASKPASS="fluxgit-git-askpass"
SETSID="fluxgit-git-setsid"
BUT="but"

if [ -f "$TARGET_ROOT/$ASKPASS.exe" ] && [ -f "$TARGET_ROOT/$SETSID.exe" ]; then
    log injecting windows fluxgit-git binaries into crates/fluxgit-tauri "(TRIPLE=${TRIPLE})"
    ...
    log injecting fluxgit-git binaries into crates/fluxgit-tauri "(TRIPLE=${TRIPLE})"
    cp -v "$TARGET_ROOT/$ASKPASS" "$CRATE_ROOT/$ASKPASS-${TRIPLE}"
    cp -v "$TARGET_ROOT/$SETSID" "$CRATE_ROOT/$SETSID-${TRIPLE}"
else
    log fluxgit-git binaries are not built
    exit 1
fi
