pub(crate) mod state {
    ...
    struct State {
        ...
        watcher: fluxgit_watcher::WatcherHandle,
    ...
    fn handler_from_app(app_handle: &AppHandle) -> Result<fluxgit_watcher::Handler> {
        Ok(fluxgit_watcher::Handler::new({
    ...
    impl WindowState {
        ...
        pub fn set_project_to_window(
            ...
            let watch_mode = fluxgit_watcher::WatchMode::from_env_or_settings(
                &app_settings.get()?.feature_flags.watch_mode,
                |key| std::env::var(key).ok(),
            );
            let watcher = fluxgit_watcher::watch_in_background(
                handler,
                worktree_dir,
                project_id,
                app_settings.clone(),
