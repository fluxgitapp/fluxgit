...
fn main() -> anyhow::Result<()> {
    ...
    let performance_logging = std::env::var_os("FLUXGIT_PERFORMANCE_LOG").is_some();
    let tauri_debug_logging = std::env::var_os("FLUXGIT_TAURI_DEBUG_LOG").is_some();

    fluxgit_project::configure_git2();
    ...
    if let Some(project_to_open) =
        std::env::var_os("FLUXGIT_PROJECT_DIR").map(std::path::PathBuf::from)
    {
        bail!(
    ...
    let tokio_debug = matches!(std::env::var("FLUXGIT_TOKIO_DEBUG").as_deref(), Ok("1"));
    ...
    runtime.block_on(async {
        ...
        let builder = tauri::Builder::default()
            .setup(move |tauri_app| {
                let window = fluxgit_tauri::window::create(
                    tauri_app.handle(),
                    "main",
                    "index.html".into(),
                )
                ...
                if tauri_app.config().product_name != Some("FluxGit Test".to_string()) {
                ...
                unsafe {
                    fluxgit_repo_actions::askpass::init({
                ...
                app_settings.watch_in_background({
                    ...
                    move |app_settings| {
                        fluxgit_tauri::ChangeForFrontend::from(app_settings).send(&app_handle)
                    }
                })?;
...
fn migrate_projects() -> anyhow::Result<()> {
    for mut project in fluxgit_project::dangerously_list_projects_without_migration()? {
        if let Ok(true) = project.migrate() {
            let (title, git_dir) = (project.title.clone(), project.git_dir().to_owned());
            if let Err(err) = fluxgit_project::update(project.into()) {
                tracing::warn!(
