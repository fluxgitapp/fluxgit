...
fn should_log(level: Option<Level>, meta: &tracing::Metadata<'_>) -> bool {
    ...
    meta.module_path().is_none_or(|p| {
        p.starts_with("fluxgit_") || p.starts_with("but::") || p.starts_with("but_")
    })
}
...
fn get_server_addr(app_handle: &AppHandle) -> (Ipv4Addr, u16) {
    ...
    let port = if product_name.eq("FluxGit") {
        6667
    } else if product_name.eq("FluxGit Nightly") {
        6668
    } else {
        6669
    };
...
fn remove_old_logs(log_directory: &Path) -> anyhow::Result<()> {
    ...
    let old_log_files = dir.filter_map(|entry| {
        ...
        let filename = entry.file_name();
        let filename = filename.to_str()?;
        if !filename.starts_with("FluxGit.log") {
            return None;
        }
