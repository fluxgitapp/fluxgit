...
pub fn set_project_active(
    ...
    let project = match fluxgit_project::get_validated(id).ok() {
...
Ensure these aren't touched by FluxGit or avoid using it in this repository.",
...
`git lfs pull --include="*"` can be used to restore git-lfs files after FluxGit touched them."#,
        );
    }
    let max_files = 10;
    msg.push_str("\n\n");
