...
use fluxgit_repo::{FLUXGIT_COMMIT_AUTHOR_EMAIL, FLUXGIT_COMMIT_AUTHOR_NAME};
...
///
/// How it works:
/// First a reference fluxgit/target is created, pointing to the head of the target (trunk) branch.
/// This is a fake branch that we don't need to care about. If it doesn't exist, it is created.
/// Then in the reflog entry logs/refs/heads/fluxgit/target we pretend that the ref originally pointed to the
...
fn standard_signature() -> gix::actor::Signature {
    gix::actor::Signature {
        name: FLUXGIT_COMMIT_AUTHOR_NAME.into(),
        email: FLUXGIT_COMMIT_AUTHOR_EMAIL.into(),
...
mod set_target_ref {
    ...
    use super::{
        FLUXGIT_COMMIT_AUTHOR_EMAIL, FLUXGIT_COMMIT_AUTHOR_NAME, ReflogCommits,
    ...
    fn reflog_present_but_empty() -> anyhow::Result<()> {
        ...
        let log_file_path = worktree_dir.join(".git/logs/refs/heads/fluxgit/target");
    ...
    fn reflog_present_but_branch_is_missing() -> anyhow::Result<()> {
        ...
        let loose_ref_path = worktree_dir.join(".git/refs/heads/fluxgit/target");
    ...
    fn assert_signature(sig: gix::actor::SignatureRef<'_>) {
        assert_eq!(sig.name, FLUXGIT_COMMIT_AUTHOR_NAME);
        assert_eq!(sig.email, FLUXGIT_COMMIT_AUTHOR_EMAIL);
        assert_ne!(
            sig.seconds(),
            0,
