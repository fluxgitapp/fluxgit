...
use git2::FileMode;
use fluxgit_cherry_pick::RepositoryExtLite;
use fluxgit_repo::{RepositoryExt as _, SignaturePurpose};
use fluxgit_stack::{VirtualBranchesHandle, VirtualBranchesState};
...
/// Snapshots include the state of the working directory as well as all additional FluxGit state (e.g. virtual branches, conflict state).
...
pub trait OplogExt {
    ...
    /// Prepares a snapshot of the current state of the working directory as well as FluxGit data.
    ...
    /// Creates a snapshot of the current state of the working directory as well as FluxGit data.
    ...
    ///
    /// An alternative way of retrieving the snapshots would be to manually the oplog head `git log <oplog_head>` available in `.git/fluxgit/operations-log.toml`.
    ///
...
pub fn prepare_snapshot(ctx: &Context, _shared_access: &RepoShared) -> Result<git2::Oid> {
    ...
    // Create a blob out of `.git/fluxgit/virtual_branches.toml`
    ...
    // also add the fluxgit/workspace commit to the branches tree
    let head = repo.head()?;
    if head.name() == Some("refs/heads/fluxgit/workspace") {
    ...
    for branch_entry in walker {
        ...
        for commit_entry in commits_tree.iter() {
            ...
            if let Some(commit_id) = commit_entry.name() {
                ...
                // if branch_name is 'workspace', we need to create or update the fluxgit/workspace branch
                if branch_name == Some("workspace") {
                    ...
                    git2_repo.branch("fluxgit/workspace", &workspace_commit, true)?;
                    // make sure head is fluxgit/workspace
                    git2_repo.set_head("refs/heads/fluxgit/workspace")?;
                }
            }
        }
    }
