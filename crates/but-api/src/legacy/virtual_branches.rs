...
use fluxgit_branch::{BranchCreateRequest, BranchUpdateRequest};
use fluxgit_branch_actions::{
...
use fluxgit_project::FetchResult;
use fluxgit_reference::{Refname, RemoteRefname, normalize_branch_name as normalize_name};
...
pub fn delete_local_branch(
    ...
    fluxgit_branch_actions::delete_local_branch(ctx, &refname, given_name)?;
    Ok(())
}
...
) -> Result<fluxgit_branch_actions::CreateBranchFromBranchOutcome> {
    let outcome = fluxgit_branch_actions::create_virtual_branch_from_branch(
...
pub fn integrate_upstream_commits(
    ...
    fluxgit_branch_actions::integrate_upstream_commits(
        ctx,
        stack_id,
        series_name,
...
pub fn get_initial_integration_steps_for_branch(
    ...
    Vec<fluxgit_branch_actions::branch_upstream_integration::InteractiveIntegrationStep>,
    Error,
> {
    let steps = fluxgit_branch_actions::branch_upstream_integration::get_initial_integration_steps_for_branch(
        ctx,
        stack_id,
        branch_name,
    )?;
...
pub fn integrate_branch_with_steps(
    ...
    stack_id: StackId,
    branch_name: String,
    steps: Vec<fluxgit_branch_actions::branch_upstream_integration::InteractiveIntegrationStep>,
) -> Result<()> {
    fluxgit_branch_actions::integrate_branch_with_steps(ctx, stack_id, branch_name, steps)
}
...
pub fn switch_back_to_workspace_with_perm(
    ...
    let base_branch = fluxgit_branch_actions::base::get_base_branch_data(ctx)
    ...
    fluxgit_branch_actions::set_base_branch(ctx, &branch_name, perm)?;
...
pub fn get_base_branch_data(ctx: &but_ctx::Context) -> Result<Option<BaseBranch>> {
    if let Ok(base_branch) = fluxgit_branch_actions::base::get_base_branch_data(ctx) {
        Ok(Some(base_branch))
    } else {
        Ok(None)
    }
...
pub fn set_base_branch_with_perm(
    ...
    let branch_name = format!("refs/remotes/{branch}")
        .parse()
        .context("Invalid branch name")?;
    let base_branch = fluxgit_branch_actions::set_base_branch(ctx, &branch_name, perm)?;

    // if they also sent a different push remote, set that too
    if let Some(push_remote) = push_remote {
        fluxgit_branch_actions::set_target_push_remote(ctx, &push_remote)?;
    }
    {
        crate::legacy::meta::reconcile_in_workspace_state_of_vb_toml(ctx, perm).ok();
    }
...
pub fn push_base_branch(ctx: &mut but_ctx::Context, with_force: bool) -> Result<()> {
    fluxgit_branch_actions::push_base_branch(ctx, with_force)?;
    Ok(())
}
...
pub fn update_stack_order(
    ...
    fluxgit_branch_actions::update_stack_order(ctx, stacks)?;
    Ok(())
}
...
pub fn unapply_stack(ctx: &mut Context, stack_id: StackId) -> Result<()> {
    ...
    drop((repo, ws, db));
    fluxgit_branch_actions::unapply_stack(
        ctx,
...
pub fn amend_virtual_branch(
    ...
    let oid =
        fluxgit_branch_actions::amend(ctx, stack_id, commit_id.to_git2(), worktree_changes)?;
    Ok(oid.to_string())
}
...
pub fn undo_commit(
    ...
    fluxgit_branch_actions::undo_commit(ctx, stack_id, commit_id.to_git2())?;
    Ok(())
}
...
pub fn reorder_stack(
    ...
    fluxgit_branch_actions::reorder_stack(ctx, stack_id, stack_order)?;
    Ok(())
}
...
pub fn list_branches(
    ...
    let branches = fluxgit_branch_actions::list_branches(ctx, filter, None)?;
    Ok(branches)
}
...
pub fn get_branch_listing_details(
    ...
    let branches = fluxgit_branch_actions::get_branch_listing_details(ctx, branch_names)?;
    Ok(branches)
}
...
pub fn squash_commits(
    ...
    let source_commit_ids: Vec<git2::Oid> = source_commit_ids
        ...
        .collect::<Result<_, _>>()
        .map_err(|e| anyhow!(e))?;
    fluxgit_branch_actions::squash_commits(
        ctx,
        stack_id,
        source_commit_ids,
...
pub fn fetch_from_remotes(ctx: &Context, action: Option<String>) -> Result<BaseBranch> {
    let project_data_last_fetched = fluxgit_branch_actions::fetch_from_remotes(
        ctx,
    ...
    fluxgit_project::update(fluxgit_project::UpdateRequest {
        project_data_last_fetched: Some(project_data_last_fetched.clone()),
        ..fluxgit_project::UpdateRequest::default_with_id(ctx.legacy_project.id)
    })
...
pub fn move_commit(
    ...
    fluxgit_branch_actions::move_commit(
        ctx,
        target_stack_id,
        commit_id.to_git2(),
        source_stack_id,
...
pub fn move_branch(
    ...
    fluxgit_branch_actions::move_branch(
        ctx,
        target_stack_id,
...
pub fn tear_off_branch(
    ...
    fluxgit_branch_actions::tear_off_branch(ctx, source_stack_id, subject_branch_name.as_str())
}
...
pub fn update_commit_message(
    ...
    let new_commit_id = fluxgit_branch_actions::update_commit_message(
        ctx,
        stack_id,
        commit_id.to_git2(),
        &message,
...
pub async fn upstream_integration_statuses(
    ...
    let (base_branch, commit_id, ctx) = {
        ...
        (
            fluxgit_branch_actions::base::get_base_branch_data(&ctx)?,
            commit_id,
            ctx.into_sync(),
        )
    };
    ...
    fluxgit_branch_actions::upstream_integration_statuses(&mut ctx, commit_id, &resolved_reviews)
}
...
pub async fn integrate_upstream(
    ...
    let resolved_reviews = resolve_review_map(ctx.clone(), &base_branch).await?;
    let mut ctx = ctx.into_thread_local();
    let outcome = fluxgit_branch_actions::integrate_upstream(
        &mut ctx,
        &resolutions,
...
pub async fn resolve_upstream_integration(
    ...
    let new_target_id = fluxgit_branch_actions::resolve_upstream_integration(
        &mut ctx,
        resolution_approach,
        &resolved_reviews,
    )?;
