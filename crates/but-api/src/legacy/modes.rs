...
use fluxgit_edit_mode::ConflictEntryPresence;
use fluxgit_operating_modes::{EditModeMetadata, OperatingMode};
...
pub fn operating_mode(ctx: &Context) -> Result<HeadAndMode, Error> {
    ...
    Ok(HeadAndMode {
        head: head_ref_short,
        operating_mode: Some(fluxgit_operating_modes::operating_mode(ctx)),
    })
}
...
pub fn enter_edit_mode(
    ...
    fluxgit_edit_mode::commands::enter_edit_mode(ctx, commit_id.to_git2(), stack_id)
}

#[but_api]
#[instrument(err(Debug))]
pub fn abort_edit_and_return_to_workspace(ctx: &mut but_ctx::Context, force: bool) -> Result<()> {
    fluxgit_edit_mode::commands::abort_and_return_to_workspace(ctx, force)?;
...
pub fn save_edit_and_return_to_workspace(ctx: &mut but_ctx::Context) -> Result<()> {
    fluxgit_edit_mode::commands::save_and_return_to_workspace(ctx)?;
...
pub fn edit_initial_index_state(
    ...
    fluxgit_edit_mode::commands::starting_index_state(ctx)
}

#[but_api]
#[instrument(err(Debug))]
pub fn edit_changes_from_initial(ctx: &mut but_ctx::Context) -> Result<Vec<TreeChange>> {
    fluxgit_edit_mode::commands::changes_from_initial(ctx)
}
