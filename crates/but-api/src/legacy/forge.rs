...
pub fn review_template(ctx: &Context) -> Result<Option<json::ReviewTemplateInfo>> {
    let project = fluxgit_project::get_validated(ctx.legacy_project.id)?;
    ...
    match repo.git_settings()?.fluxgit_forge_review_template_path {
...
pub fn set_review_template(ctx: &but_ctx::Context, template_path: Option<String>) -> Result<()> {
    ...
    git_config.fluxgit_forge_review_template_path = template_path.map(|p| p.into());
