//! FluxGit has an operation log (oplog) that records significant actions taken within a project.
...
//! Refer to `fluxgit_oplog::entry::Snapshot` and `fluxgit_oplog::entry::SnapshotDetails` for the metadata stored.
//!
use anyhow::Result;
