...
use fluxgit_commit::commit_ext::CommitExt;
use fluxgit_oplog::{
    OplogExt, SnapshotExt,
    entry::{OperationKind, SnapshotDetails},
};
use fluxgit_reference::{LocalRefname, Refname};
use fluxgit_stack::{StackId, VirtualBranchesHandle};
...
pub fn stack_details(
    ...
    let gerrit_mode = repo.git_settings()?.fluxgit_gerrit_mode.unwrap_or(false);
    let db = ctx.db.get()?;
    if gerrit_mode {
...
pub fn stash_into_branch(
    ...
    let stack = branch_manager.create_virtual_branch(
        &fluxgit_branch::BranchCreateRequest {
    ...
    fluxgit_branch_actions::update_workspace_commit(&vb_state, ctx, false)
        .context("failed to update fluxgit workspace")?;
...
/// with remote tracking branches by name, not only by configuration, to support older FluxGit setups.
///
