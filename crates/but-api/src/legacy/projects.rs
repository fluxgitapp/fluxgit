...
use fluxgit_project::{self as projects, ProjectId};
use tracing::instrument;

#[but_api]
#[instrument(err(Debug))]
pub fn update_project(project: projects::UpdateRequest) -> Result<projects::api::Project> {
    Ok(fluxgit_project::update(project)?.into())
}

/// Adds an existing git repository as a FluxGit project.
...
pub fn add_project_best_effort(path: PathBuf) -> Result<projects::AddProjectOutcome> {
    fluxgit_project::add_with_best_effort(&path)
}
...
pub fn get_project(
    ...
    if no_validation.unwrap_or(false) {
        Ok(fluxgit_project::get_raw(project_id)?.migrated()?.into())
    } else {
        Ok(fluxgit_project::get_validated(project_id)?.into())
    }
}
...
pub fn list_projects(opened_projects: Vec<ProjectId>) -> Result<Vec<ProjectForFrontend>> {
    fluxgit_project::assure_app_can_startup_or_fix_it(
        fluxgit_project::dangerously_list_projects_without_migration(),
    )
    .map(|projects| {
        projects
            .into_iter()
...
pub fn delete_project(project_id: ProjectId) -> Result<()> {
    fluxgit_project::delete(project_id)
}

#[but_api]
#[instrument(err(Debug))]
pub fn is_gerrit(ctx: &but_ctx::Context) -> Result<bool> {
    fluxgit_project::gerrit::is_used_by_default_remote(&*ctx.repo.get()?)
}
...
pub struct ProjectForFrontend {
    #[serde(flatten)]
    pub inner: fluxgit_project::api::Project,
