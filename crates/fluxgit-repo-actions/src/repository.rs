        ...
        if use_git_executable {
            ...
            match std::thread::spawn(move || {
                tokio::runtime::Runtime::new()
                    .unwrap()
                    .block_on(fluxgit_git::push(
                        repo_path,
                        fluxgit_git::tokio::TokioExecutor,
                        &remote,
                        fluxgit_git::RefSpec::parse(refspec).unwrap(),
                        with_force,
            ...
            .unwrap() {
                Ok(result) => Ok(result),
                Err(err) => match err {
                    fluxgit_git::Error::ForcePushProtection(e) => {
                    ...
                    fluxgit_git::Error::GerritNoNewChanges(_) => {
