...
use fluxgit_branch_actions::{BranchManagerExt, update_workspace_commit};
use fluxgit_oplog::{
    OplogExt, SnapshotExt,
    entry::{OperationKind, SnapshotDetails},
};
use fluxgit_reference::{LocalRefname, Refname};
use fluxgit_stack::{PatchReferenceUpdate, VirtualBranchesHandle};
...
pub fn create_commit(
    ...
    let stack_id = stacks
        ...
        .unwrap_or_else(|| {
            ...
            let branch = fluxgit_branch::BranchCreateRequest {
                name: Some(params.branch_name.clone()),
                ..Default::default()
            };

            let stack = fluxgit_branch_actions::create_virtual_branch(ctx, &branch, perm)
...
pub fn create_branch(
    ...
    let stack_entry = fluxgit_branch_actions::create_virtual_branch(ctx, &branch, perm)?;
...
fn move_file_changes(
    ...
    let vb_state = VirtualBranchesHandle::new(ctx.project_data_dir());
    fluxgit_branch_actions::update_workspace_commit(&vb_state, ctx, false)?;
...
pub fn squash_commits(
    ...
    let squashed_commit =
        fluxgit_branch_actions::squash_commits(ctx, stack_id, source_ids, destination_id)?;
