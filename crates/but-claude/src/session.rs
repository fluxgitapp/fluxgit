//! Claude bridge.
//!
//! This module provides the integration between FluxGit's frontend and Claude Code
...
use fluxgit_stack::VirtualBranchesHandle;
...
impl Claudes {
    ...
    async fn spawn_claude_inner(
        ...
        if let Err(e) = broadcast_fluxgit_messages(
            &sync_ctx,
            &broadcaster,
            original_session_id,
            stack_id,
        ...
        .await
        {
            tracing::warn!(
                original_session_id = %original_session_id,
                error = %e,
                "Failed to broadcast FluxGit messages created during session"
            );
        }
...
/// Broadcasts any FluxGit messages created during this Claude session.
async fn broadcast_fluxgit_messages(
    sync_ctx: &ThreadSafeContext,
    broadcaster: &Arc<Mutex<Broadcaster>>,
    session_id: uuid::Uuid,
    stack_id: StackId,
    ...
    let new_messages: Vec<_> = all_messages
        .into_iter()
        .filter(|msg| matches!(msg.payload, MessagePayload::FluxGit(_)))
...
fn system_prompt() -> String {
    ...
    format!(
        "<git-usage>
CRITICAL: You are working on a project that is managed by FluxGit.
...
DO NOT mention FluxGit unless the user asks you to perform a disallowed git action.
...
<response>
Sorry, this project is managed by FluxGit so you must make commits through the FluxGit interface.
</response>
</example>
...
<response>
Sorry, this project is managed by FluxGit so you must integrate upstream changes through the FluxGit interface.
</response>
</example>
...
fn format_branch_info(ctx: &mut Context, stack_id: StackId, perm: &RepoShared) -> String {
    let mut output = String::from(
        "<branch-info>\n\
        This repository uses FluxGit for branch management. While git shows you are on\n\
        the `fluxgit/workspace` branch, this is actually a merge commit containing one or\n\
...
fn create_can_use_tool_callback(
    sync_ctx: ThreadSafeContext,
    broadcaster: Arc<Mutex<Broadcaster>>,
    stack_id: fluxgit_stack::StackId,
