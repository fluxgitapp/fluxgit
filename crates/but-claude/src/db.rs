...
impl TryFrom<but_db::ClaudeMessage> for crate::ClaudeMessage {
    ...
    fn try_from(value: but_db::ClaudeMessage) -> Result<Self, Self::Error> {
        ...
        let payload = match payload_type {
            ...
            MessagePayloadDbType::FluxGitUpdate => {
                crate::MessagePayload::FluxGit(serde_json::from_str(&value.content)?)
            }
            MessagePayloadDbType::FluxGitMessage => {
                crate::legacy::ClaudeMessageContent::FluxGitMessage(serde_json::from_str(
                    &value.content,
                )?)
                .into()
            }
...
impl TryFrom<crate::ClaudeMessage> for but_db::ClaudeMessage {
    ...
    fn try_from(value: crate::ClaudeMessage) -> Result<Self, Self::Error> {
        let (payload_type, content) = match value.payload {
            ...
            crate::MessagePayload::FluxGit(msg) => {
                let content = serde_json::to_string(&msg)?;
                (MessagePayloadDbType::FluxGitUpdate, content)
            }
        };
...
mod tests {
    ...
    #[test]
    fn test_fluxgit_message_claude_exit() {
        let db_message = but_db::ClaudeMessage {
            ...
            created_at: chrono::DateTime::from_timestamp(1234567890, 0)
                .unwrap()
                .naive_utc(),
            content_type: "FluxGitMessage".to_string(),
            content: r#"{
                "subject": {
                    "code": 0,
                    "message": ""
    ...
    #[test]
    fn test_fluxgit_message_user_abort() {
        let db_message = but_db::ClaudeMessage {
