...
pub enum ClaudeMessageContent {
    ...
    /// Inserted via  FluxGit (what the user typed)
    UserInput(UserInput),
    /// Metadata provided by FluxGit around the Claude Code status
    FluxGitMessage(FluxGitMessage),
}
...
impl From<ClaudeMessageContent> for crate::MessagePayload {
    fn from(value: ClaudeMessageContent) -> Self {
        match value {
            ...
            ClaudeMessageContent::FluxGitMessage(gb_message) => {
...
/// Metadata provided by FluxGit.
#[derive(Serialize, Deserialize, Debug, Clone)]
#[serde(rename_all = "camelCase", tag = "type", content = "subject")]
pub enum FluxGitMessage {
...
impl From<FluxGitMessage> for crate::SystemMessage {
    fn from(value: FluxGitMessage) -> Self {
        match value {
            FluxGitMessage::ClaudeExit { code, message } => {
                crate::SystemMessage::ClaudeExit { code, message }
            }
            FluxGitMessage::UserAbort => crate::SystemMessage::UserAbort,
            FluxGitMessage::UnhandledException { message } => {
                crate::SystemMessage::UnhandledException { message }
            }
            FluxGitMessage::CompactStart => crate::SystemMessage::CompactStart,
            FluxGitMessage::CompactFinished { summary } => {
