...
pub fn handle_session_stop(
    ...
    // TODO: Maybe this can be done in the main app process i.e. the FluxGit GUI, if available
    ...
    if let Some(llm) = LLMProvider::default_openai() {
        for branch in &outcome.updated_branches {
            ...
            let commit_message = crate::MessagePayload::FluxGit(
                crate::FluxGitUpdate::CommitCreated(crate::CommitCreatedDetails {
...
fn create_stack(ctx: &Context, perm: &mut RepoExclusive) -> anyhow::Result<StackId> {
    ...
    let stack = fluxgit_branch_actions::create_virtual_branch(ctx, &create_req, perm)?;
    Ok(stack.id)
}
...
fn should_exit_early(ctx: &mut Context, current_id: Uuid) -> anyhow::Result<bool> {
    let in_gui = std::env::var("FLUXGIT_IN_GUI").is_ok_and(|v| v == "1");
    if in_gui {
        return Ok(false);
    }
