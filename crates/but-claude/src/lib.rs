...
/// Represents a Claude Code session that FluxGit is tracking.
...
pub enum MessagePayload {
    ...
    User(UserInput),
    /// System message from FluxGit about the session
    System(SystemMessage),
    /// Resource update, e.g. a commit was created
    FluxGit(FluxGitUpdate),
}
...
/// System messages from FluxGit about the Claude session state.
#[derive(Serialize, Deserialize, Debug, Clone)]
#[serde(rename_all = "camelCase", tag = "type")]
pub enum SystemMessage {
    ...
    /// Deprecated and will be removed, see `FluxGitUpdate::CommitCreated`.
    CommitCreated(CommitCreatedDetails),
}

/// Details about a Claude session, extracted from the Claude transcript.
/// This data is derived just in time, i.e. not persisted by FluxGit.
#[derive(Serialize, Deserialize, Debug, Clone)]
#[serde(rename_all = "camelCase")]
...
pub struct ClaudeAskUserQuestionRequest {
    ...
    pub stack_id: Option<fluxgit_stack::StackId>,
}
