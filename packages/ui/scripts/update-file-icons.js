 ...
 *
 * Usage:
 *   node scripts/update-file-icons.js <svg-dir>
 */
...
function optimizeSvg(raw) {
	...
	svg = svg.replace(/<svg([^>]*)>/, (_, attrs) => {
		...
		const xmlns = getAttr("xmlns") ?? "http://www.w3.org/2000/svg";
		const viewBox = getAttr("viewBox") ?? "0 0 14 14";
...
if (!svgDir) {
	console.error("Usage: node scripts/update-file-icons.js <svg-dir>");
	process.exit(1);
}
...
for (const file of svgFiles) {
	...
	if (patched) {
		...
		process.stdout.write(`⚠ no match "${key}" – skipping (add manually if needed)\n`);
