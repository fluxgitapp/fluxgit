...
`@fluxgit/but-sdk` is the local npm package that exposes FluxGit Rust APIs to JavaScript/TypeScript through a native Node add-on.
...
1. **N-API bindings (functions)**
   ...
   - `napi-rs` generates:
     - `fluxgit-sdk.{PLATFORM}.node` (Node native add-on)
...
```bash
pnpm --filter @fluxgit/but-sdk build:napi
pnpm --filter @fluxgit/but-sdk build:types
```

Or run the combined script:

```bash
pnpm --filter @fluxgit/but-sdk build
```
...
```bash
pnpm --filter @fluxgit/but-sdk check
```

Optional extra runtime/type sanity check:

```bash
pnpm --filter @fluxgit/but-sdk testTypes
```
...
```ts
import type { ProjectForFrontend } from '@fluxgit/but-sdk';
