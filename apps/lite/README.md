...
- `pnpm --filter @fluxgit/lite dev`: run Vite + Electron
- `pnpm --filter @fluxgit/lite build`: build renderer and Electron code
- `pnpm --filter @fluxgit/lite check`: TypeScript checks for both targets
- `pnpm --filter @fluxgit/lite package`: build and package with Electron Builder
...
## `@fluxgit/but-sdk` integration

`apps/lite` consumes Rust bindings via `@fluxgit/but-sdk` using a strict process boundary:
