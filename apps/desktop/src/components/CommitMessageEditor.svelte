<script lang="ts">
	...
	import { Button, TestId } from "@fluxgit/ui";
	import { IME_COMPOSITION_HANDLER } from "@fluxgit/ui/utils/imeHandling";
	...
	const canUseAI = $derived(
		aiMacros.canUseAI ||
		(aiProviderService.settings.useForCommitMessages && !!aiProviderService.getActiveClient())
	);

	const aiProviderTooltip = $derived.by(() => {
		const { activeProvider, activeModel, providerKeys } = aiProviderService.settings;
		if (!activeProvider) return 'Configure AI in Settings → AI options';
		const key = providerKeys[activeProvider];
		if (!key) return 'Configure AI in Settings → AI options';
		...
		return `Generate with ${providerName} ${modelName}`;
	});
	...
	async function onAiButtonClick(params: AiButtonClickParams = {}) {
		...
		try {
			...
			const newClient = aiProviderService.getActiveClient();
			if (newClient && aiProviderService.settings.useForCommitMessages) {
				const diffInput = await diffInputContext.diffInput();
				if (diffInput) {
					const diff = diffInput.map(d => `${d.filePath}\n${d.diff}`).join('\n\n');
					beginGeneration();
					try {
						const output = await newClient.generateCommitMessage(diff);
						if (output) {
						...
						showError('AI provider error', err);
					}
					return;
				}
			}
