<script lang="ts">
	...
	import { CardGroup, Link, Select, SelectItem, Toggle } from "@fluxgit/ui";
	...
	function toggleCommitterSigning() {
		...
		gitConfig.set("fluxgit.fluxgitCommitter", annotateCommits ? "1" : "0");
	}
	...
	onMount(async () => {
		annotateCommits = (await gitConfig.get("fluxgit.fluxgitCommitter")) === "1";
	});
...
<CardGroup.Item standalone labelFor="committerSigning">
	{#snippet title()}
		Credit FluxGit as the committer
	{/snippet}
	{#snippet caption()}
		By default, everything in the FluxGit client is free to use. You can opt in to crediting us as
		the committer in your virtual branch commits to help spread the word.
		<Link
			href="https://github.com/fluxgitapp/fluxgit-docs/blob/d81a23779302c55f8b20c75bf7842082815b4702/content/docs/features/virtual-branches/committer-mark.mdx"
		>
			Learn more
		</Link>
	{/snippet}
