<script lang="ts">
	...
	import { OpenAIModelName, AnthropicModelName, GeminiModelName, ModelKind } from "$lib/ai/types";
	...
	let geminiKey: string | undefined = $state();
	let geminiModelName: GeminiModelName | undefined = $state();
	...
	onMount(async () => {
		...
		geminiKey = await aiService.getGeminiKey();
		geminiModelName = await aiService.getGeminiModelName();
	...
	const geminiModelOptions = [
		{
			...
			value: GeminiModelName.Gemini25Flash,
		},
	];
	...
	run(() => {
		setConfiguration(GitAIConfigKey.GeminiModelName, geminiModelName);
	});
	run(() => {
		setSecret(AISecretHandle.GeminiKey, geminiKey);
	});
	run(() => {
...
<p class="text-13 text-body ai-settings__about-text">
	FluxGit supports multiple AI providers: Google Gemini (default), OpenAI, and Anthropic, plus
	local models through Ollama and LM Studio.
</p>

<CardGroup>
	<form class="git-radio" bind:this={form} onchange={(e) => onFormChange(e.currentTarget)}>
		<CardGroup.Item labelFor="gemini">
			{#snippet title()}
				Google Gemini
			{/snippet}
			{#snippet actions()}
				<RadioButton name="modelKind" id="gemini" value={ModelKind.Gemini} />
			{/snippet}
		</CardGroup.Item>
		{#if modelKind === ModelKind.Gemini}
			<CardGroup.Item>
				<Textbox
					...
					placeholder="Enter your Gemini API key (or set in .env)"
				/>
				...
				<Select
					...
					wide
					onselect={(value) => {
						geminiModelName = value as GeminiModelName;
					}}
				>
					{#snippet itemSnippet({ item, highlighted })}
						<SelectItem selected={item.value === geminiModelName} {highlighted}>
							{item.label}
						</SelectItem>
					{/snippet}
				</Select>
				{@render shortNote("FluxGit uses Google Gemini for commit messages.")}
			</CardGroup.Item>
		{/if}
		...
		{#if modelKind === ModelKind.OpenAI}
			<CardGroup.Item>
				...
				{#if openAIKeyOption === KeyOption.ButlerAPI}
					{#if !$user}
						<AuthorizationBanner message="Please sign in to use the FluxGit API." />
					{:else}
						{@render shortNote("FluxGit uses OpenAI API for commit messages and branch names.")}
					{/if}
				{/if}
		...
		{#if modelKind === ModelKind.Anthropic}
			<CardGroup.Item>
				...
				{#if anthropicKeyOption === KeyOption.ButlerAPI}
					...
					{:else}
						{@render shortNote(
							"FluxGit uses Anthropic API for commit messages and branch names.",
						)}
					{/if}
				{/if}
			...
			<CardGroup.Item>
				...
				<InfoMessage filled outlined={false}>
					...
					{#snippet content()}
						...
						<br />
						See the <Link href="https://docs.fluxgit.com/troubleshooting/custom-csp"
						...
						<div class="ai-settings__section-text-block">
							...
							<p>
								...
								>. You can find more details on how to do that in the <Link
									href="https://docs.fluxgit.com/troubleshooting/custom-csp">FluxGit docs</Link
								>.
							</p>
...
<SettingsSection>
	...
	{/snippet}
	{#snippet description()}
		FluxGit's AI assistant generates commit messages and branch names. Use default prompts or
