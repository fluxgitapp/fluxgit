<script lang="ts">
	...
	import { CardGroup, Textbox, Toggle } from "@fluxgit/ui";
...
<CardGroup>
	...
	{#if $user?.role === "admin"}
		<CardGroup.Item labelFor="single-branch">
			...
			{/snippet}
			{#snippet caption()}
				Stay in the workspace view when leaving the fluxgit/workspace branch.
			{/snippet}
			{#snippet actions()}
				<Toggle
					id="single-branch"
		...
		{#if $ircEnabled}
			<CardGroup.Item>
				<Textbox
					...
					size="large"
					label="Server"
					placeholder="wss://irc.fluxgit.com:443"
