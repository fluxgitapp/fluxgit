...
<CardGroup>
	<CardGroup.Item>
		{#snippet title()}
			Install the FluxGit CLI <code class="code-string">but</code>
		{/snippet}
		...
		{#snippet caption()}
			...
			{:else}
				Installs the FluxGit CLI (<code>`but`</code>) in your PATH, allowing you to use it from
...
<CardGroup>
	<CardGroup.Item>
		...
		{/snippet}
		{#snippet caption()}
			You can delete all projects from the FluxGit app.
			<br />
...
>
	<p>Are you sure you want to remove all FluxGit projects?</p>
