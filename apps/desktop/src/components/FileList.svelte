...
<script lang="ts">
	...
	import { inject, injectOptional } from "@fluxgit/core/context";
	import { AsyncButton, FileListItem, TestId } from "@fluxgit/ui";
	import { FOCUS_MANAGER } from "@fluxgit/ui/focus/focusManager";
	...
	$effect(() => {
		aiService.validateFluxGitAPIConfiguration().then((value) => {
			...
			{#if ancestorMostConflictedCommitId}
				<div class="conflicted-entries__action">
					<p class="text-12 text-body clr-text-2">
						If the branch has multiple conflicted commits, FluxGit opens the earliest one first,
