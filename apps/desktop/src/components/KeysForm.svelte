<script lang="ts">
	...
	import { CardGroup, Link, RadioButton, Textbox } from "@fluxgit/ui";
	...
	<ReduxResult {projectId} result={projectQuery.result}>
		{#snippet children(project)}
			<SettingsSection>
				...
				{/snippet}
				{#snippet description()}
					Configure the authentication flow for FluxGit when authenticating with your Git remote
					provider.
				{/snippet}
				<CardGroup>
					<form
					...
					>
						...
						<CardGroup.Item labelFor="credential-local">
							...
							{#snippet caption()}
								{#if selectedType === "local"}
									Add the path to an existing SSH key that FluxGit can use.
								{/if}
							{/snippet}
						</CardGroup.Item>
						...
						<CardGroup.Item labelFor="credential-helper">
							...
							{#snippet caption()}
								{#if selectedType === "gitCredentialsHelper"}
									FluxGit will use the system's git credentials helper.
