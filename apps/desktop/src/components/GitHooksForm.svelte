<script lang="ts">
	...
	import { CardGroup, Toggle } from "@fluxgit/ui";
...
<SettingsSection>
	<CardGroup>
		<CardGroup.Item labelFor="runHooks">
			...
			{/snippet}
			{#snippet caption()}
				Enable running git hooks (pre-push, pre/post-commit, commit-msg) during FluxGit actions.
			{/snippet}
			{#snippet actions()}
			...
			<CardGroup>
				<CardGroup.Item labelFor="huskyHooks">
					...
					{#snippet caption()}
						...
						<br />
						Allow FluxGit to execute scripts from `.husky` (which can come from the repository). Hooks
