<script lang="ts">
	...
	import VirtualList from "@fluxgit/ui/components/VirtualList.svelte";
	...
	const aiProviderLabel = $derived.by(() => {
		const { activeProvider, activeModel, providerKeys, useForAgentChat } = aiProviderService.settings;
		if (!useForAgentChat) return null;
		if (!activeProvider || !providerKeys[activeProvider]) return 'Configure AI in Settings → AI options';
			...
			<PreviewHeader {onclose}>
				{#snippet content()}
					<div class="chat-header-title">
						<h3 class="text-14 text-semibold truncate">Chat for {branchName}</h3>
						{#if aiProviderLabel}
							<span class="text-11 clr-text-2 truncate">{aiProviderLabel}</span>
						{/if}
					</div>
				{/snippet}
