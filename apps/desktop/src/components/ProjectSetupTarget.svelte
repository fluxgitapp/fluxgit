<script lang="ts">
	...
	import { Button, CardGroup, Icon, Link, Select, SelectItem, TestId, Toggle } from "@fluxgit/ui";
	...
	const gerritMode = $derived(gbConfig.response?.fluxgitGerritMode ?? false);
	...
	<div class="project-setup__fields">
		<div class="project-setup__field-wrap" data-testid={TestId.ProjectSetupPageTargetBranchSelect}>
			...
			<p class="text-12 text-body project-setup__field-caption">
				...
				<br />
				<Link href="https://docs.fluxgit.com/overview#target-branch">Learn more</Link>
			</p>
		</div>
		...
		<div class="stack-v gap-6 full-width">
			<div class="project-setup__info__title">
				...
				<h3 class="text-13 text-body text-semibold">
					FluxGit switches your active branch to <span class="text-bold">fluxgit/workspace</span
					>
				</h3>
			</div>
			...
			{#if showMoreInfo}
				<p class="text-12 text-body" transition:slide={{ duration: 200 }}>
					In order to support working on multiple branches simultaneously, FluxGit creates and
					automatically manages a special branch <span class="text-bold">fluxgit/workspace</span>.
					You can always switch back and forth as needed between normal git branches and the
					Gitbutler workspace.
					<Link href="https://docs.fluxgit.com/features/branch-management/integration-branch"
						>Learn more</Link
					>
				</p>
			{/if}
