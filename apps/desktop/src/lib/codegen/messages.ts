...
import { isDefined } from "@fluxgit/ui/utils/typeguards";
import type {
	ClaudeMessage,
		...
		if (payload.source === "user") {
			...
			if (payload.type === "claudeExit" && payload.code !== 0) {
				if (previousEventLoginFailureQuery(events, message)) {
					const msg = `Claude Code is currently not logged in.\n\n Please run \`claude\` in your terminal and complete the login flow in order to use the FluxGit Claude Code integration.`;
					out.push({
						source: "claude",
						createdAt: message.createdAt,
						message: msg,
...
export function userFeedbackStatus(messages: Message[]): UserFeedbackStatus {
	...
	const toolCallsPendingApproval = "toolCallsPendingApproval" in lastMessage ? lastMessage.toolCallsPendingApproval : [];
	if (toolCallsPendingApproval.length > 0) {
		return { waitingForFeedback: true, toolCall: toolCallsPendingApproval[0]! };
	}
	let msSpentWaiting = 0;
	const toolCalls = "toolCalls" in lastMessage ? lastMessage.toolCalls : [];
