...
import type { GitConfigSettings } from "@fluxgit/core/api";
...
export class GitConfigService {
	...
	async setGbConfig(projectId: string, config: Partial<GitConfigSettings>) {
		return await this.api.endpoints.setGbConfig.mutate({
			...
			config: {
				...
				fluxgitGerritMode: config.fluxgitGerritMode ?? null,
				fluxgitForgeReviewTemplatePath: config.fluxgitForgeReviewTemplatePath ?? null,
				fluxgitGitlabProjectId: config.fluxgitGitlabProjectId ?? null,
				fluxgitGitlabUpstreamProjectId: config.fluxgitGitlabUpstreamProjectId ?? null,
	...
	async setGerritMode(projectId: string, gerritMode: boolean) {
		return await this.setGbConfig(projectId, { fluxgitGerritMode: gerritMode });
	}

	async setForgeReviewTemplatePath(projectId: string, path: string | null) {
		return await this.setGbConfig(projectId, { fluxgitForgeReviewTemplatePath: path });
	}
