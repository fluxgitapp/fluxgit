...
import { chipToasts } from "@fluxgit/ui";
import { get } from "svelte/store";
import type { IBackend } from "$lib/backend";
import type { ClientState } from "$lib/state/clientState.svelte";
import type { ForgeUser } from "@fluxgit/core/api";
...
export class ProjectsService {
	...
	validateProjectPath(path: string) {
		if (/^\\\\wsl.localhost/i.test(path)) {
			const errorMsg =
				"For WSL2 projects, install the Linux version of FluxGit inside of your WSL2 distro";
			console.error(errorMsg);
			showError("Use the Linux version of FluxGit", errorMsg);
