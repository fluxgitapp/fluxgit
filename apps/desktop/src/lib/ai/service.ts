...
import type { HttpClient } from "@fluxgit/shared/network/httpClient";
...
export enum GitAIConfigKey {
	ModelProvider = "fluxgit.aiModelProvider",
	OpenAIKeyOption = "fluxgit.aiOpenAIKeyOption",
	OpenAIModelName = "fluxgit.aiOpenAIModelName",
	OpenAICustomEndpoint = "fluxgit.aiOpenAICustomEndpoint",
	AnthropicKeyOption = "fluxgit.aiAnthropicKeyOption",
	AnthropicModelName = "fluxgit.aiAnthropicModelName",
	DiffLengthLimit = "fluxgit.diffLengthLimit",
	OllamaEndpoint = "fluxgit.aiOllamaEndpoint",
	OllamaModelName = "fluxgit.aiOllamaModelName",
	LMStudioEndpoint = "fluxgit.aiLMStudioEndpoint",
	LMStudioModelName = "fluxgit.aiLMStudioModelName",
	GeminiModelName = "fluxgit.aiGeminiModelName",
}
...
export class AIService {
	...
	async getGeminiKey() {
		// Priority: .env > SecretsService
		const envKey = (import.meta as any).env?.GEMINI_API_KEY;
		if (envKey && envKey !== "your_api_key_here") return envKey;
		return await this.secretsService.get(AISecretHandle.GeminiKey);
	}

	async getGeminiModelName() {
		return await this.gitConfig.getWithDefault<GeminiModelName>(
			GitAIConfigKey.GeminiModelName,
			GeminiModelName.Gemini25Flash,
		);
	}

	async usingFluxGitAPI() {
	...
	async validateConfiguration(): Promise<boolean> {
		...
		if (await this.usingFluxGitAPI()) return !!get(this.tokenMemoryService.token);
		...
		const geminiActiveAndKeyProvided =
			modelKind === ModelKind.Gemini && !!(await this.getGeminiKey());
		...
		return (
			...
			geminiActiveAndKeyProvided
		);
	}

	async validateFluxGitAPIConfiguration(): Promise<boolean> {
		if (!(await this.usingFluxGitAPI())) {
			return false;
		}
	...
	async buildClient(): Promise<AIClient | undefined> {
		...
		if (await this.usingFluxGitAPI()) {
			...
			if (!get(this.tokenMemoryService.token)) {
				throw new Error("When using FluxGit's API to summarize code, you must be logged in");
			}
		...
		if (modelKind === ModelKind.Gemini) {
			const geminiModelName = await this.getGeminiModelName();
			const geminiKey = await this.getGeminiKey();

			if (!geminiKey) {
				throw new Error(
					"When using Gemini in a bring your own key configuration, you must provide a valid token",
				);
			}

			return new GeminiClient(geminiKey, geminiModelName);
		}
