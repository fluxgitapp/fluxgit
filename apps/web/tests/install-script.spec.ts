...
test.describe("Install script endpoint", () => {
	test("serves install.sh script", async ({ request }) => {
		...
		expect(scriptContent).toContain("FluxGit installer bootstrap script");
		expect(scriptContent).toContain("https://app.fluxgit.com/installers/info");
		expect(scriptContent).toContain("https://releases.fluxgit.com");
