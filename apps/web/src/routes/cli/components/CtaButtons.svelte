<script lang="ts">
	...
	function handleCopy() {
		navigator.clipboard.writeText("curl -fsSL https://fluxgit.com/install.sh | sh");
		copied = true;
		setTimeout(() => {
			copied = false;
		}, 2000);
...
<section class="cta-wrap" class:dark-mode={darkMode}>
	<button type="button" class="copy-button" class:copied onclick={handleCopy}>
		...
		{#if isLinux && cliBinaryUrl}
			<code class="subtitle-text">curl -fsSL https://fluxgit.com/install.sh | sh</code>
		...
		{:else}
			<code class="subtitle-text-pop op-80">curl -fsSL https://fluxgit.com/install.sh | sh</code>
		{/if}
	...
	<a class="docs-button" href="https://docs.fluxgit.com/cli-overview" target="_blank">
		<svg
