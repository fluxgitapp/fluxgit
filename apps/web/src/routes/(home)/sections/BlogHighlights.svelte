<script lang="ts">
	...
	async function fetchRSSFeed() {
		try {
			const response = await fetch("https://blog.fluxgit.com/rss/3/featured");
			...
			items.forEach((item) => {
				...
				const creator = authorMatch ? authorMatch[1] : "FluxGit Team";

				// Extract image from enclosure tag
				const enclosure = item.querySelector("enclosure");
				const feature_image =
					enclosure?.getAttribute("url") ||
					"https://blog.fluxgit.com/content/images/2023/10/fluxgit-og.png";
...
<section class="blog-highlights">
	<SectionHeader
		...
		{#snippet buttons()}
			<ArrowButton
				label="Read more"
				onclick={() => window.open("https://blog.fluxgit.com", "_blank")}
			/>
		{/snippet}
	</SectionHeader>
