<script lang="ts">
	...
	import BranchStatusBadge from "@fluxgit/shared/branches/BranchStatusBadge.svelte";
	import Minimap from "@fluxgit/shared/branches/Minimap.svelte";
	import { BRANCH_SERVICE } from "@fluxgit/shared/branches/branchService";
	import { getBranchReview } from "@fluxgit/shared/branches/branchesPreview.svelte";
	import { lookupLatestBranchUuid } from "@fluxgit/shared/branches/latestBranchLookup.svelte";
	import { LATEST_BRANCH_LOOKUP_SERVICE } from "@fluxgit/shared/branches/latestBranchLookupService";
	import { BranchStatus, type Branch } from "@fluxgit/shared/branches/types";
	import { getContributorsWithAvatars } from "@fluxgit/shared/contributors";
	import Loading from "@fluxgit/shared/network/Loading.svelte";
	import { isFound, and, isError, map } from "@fluxgit/shared/network/loadable";
	import { APP_STATE } from "@fluxgit/shared/redux/store.svelte";
	import {
		WEB_ROUTES_SERVICE,
		type ProjectReviewParameters,
	} from "@fluxgit/shared/routing/webRoutes.svelte";
	import { UPLOADS_SERVICE } from "@fluxgit/shared/uploads/uploadsService";
	...
	import FileUploadPlugin, {
		type DropFileResult,
	} from "@fluxgit/ui/richText/plugins/FileUpload.svelte";
...
<svelte:head>
	{#if isFound(branch?.current)}
		...
		<meta property="og:title" content="FluxGit Review: {branch.current.value?.title}" />
		<meta property="og:description" content="FluxGit code review" />
	{:else}
		<title>{data.ownerSlug}/{data.projectSlug}</title>
		<meta property="og:title" content="FluxGit Review: {data.ownerSlug}/{data.projectSlug}" />
		<meta property="og:description" content="FluxGit code review" />
	{/if}
</svelte:head>
