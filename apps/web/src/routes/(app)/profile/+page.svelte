<script lang="ts">
	...
	import { getRecentlyPushedProjects } from "@fluxgit/shared/organizations/projectsPreview.svelte";
	import { APP_STATE } from "@fluxgit/shared/redux/store.svelte";
	import { NOTIFICATION_SETTINGS_SERVICE } from "@fluxgit/shared/settings/notificationSettingsService";
	import { getNotificationSettingsInterest } from "@fluxgit/shared/settings/notificationSetttingsPreview.svelte";
	import { Button, CardGroup, chipToasts, Icon, Modal, Spacer } from "@fluxgit/ui";
	...
	const downloadButtonText = $derived(`Download FluxGit for ${detectedOS}`);
			...
			{#if $user}
				...
				<CardGroup>
					<CardGroup.Item>
						...
						{/snippet}
						{#snippet caption()}
							Your access token is used to authenticate the FluxGit clients with our API.
							<br />
