<script lang="ts">
	...
	import { CardGroup, Spacer } from "@fluxgit/ui";
	...
	<div class="ssh-token-modal-backdrop" onclick={closeSshKeyTokenModal}>
		<div class="ssh-token-modal" onclick={handleModalClick}>
			...
			<p class="ssh-token-modal__description">
				Run the following command in your terminal to add your SSH key to FluxGit:
			</p>
			<div class="ssh-token-modal__code">
				<code>ssh git@ssh.fluxgit.com add/{sshKeyToken}</code>
				<button
					type="button"
					class="ssh-token-modal__copy-button"
					onclick={() => {
						navigator.clipboard.writeText(`ssh git@ssh.fluxgit.com add/${sshKeyToken}`);
					}}
				>
					Copy
				</button>
