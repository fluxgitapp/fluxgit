<script lang="ts">
	...
	import { PROJECT_SERVICE, ProjectService } from "@fluxgit/shared/organizations/projectService";
	import { APP_STATE } from "@fluxgit/shared/redux/store.svelte";
	import { WebRoutesService, WEB_ROUTES_SERVICE } from "@fluxgit/shared/routing/webRoutes.svelte";
	...
	$effect(() => {
		if (isMarketingPage) {
			...
			fetch("https://app.fluxgit.com/api/downloads?limit=1&channel=release")
			...
			fetch("https://app.fluxgit.com/api/downloads?limit=10&channel=release")
				.then((response) => response.json())
				.then((data) => {
...
<svelte:head>
	{#if import.meta.env.MODE !== "development"}
		<script
			async
			src="https://u.fluxgit.com/script.js"
