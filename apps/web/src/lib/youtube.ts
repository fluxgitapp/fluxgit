...
function mapAPIToYouTubeVideo(apiVideo: APIYouTubeVideo): YouTubeVideo {
	return {
		...
		channelTitle: "FluxGit", // Static since we know the channel
...
export async function fetchPlaylistVideos(playlistId: string): Promise<YouTubePlaylist> {
	try {
		...
		if (response.ok) {
			...
			return {
				id: playlistId,
				title: "FluxGit Feature Updates",
				description: "Latest FluxGit tutorials, feature demonstrations, and updates",
				videos,
			};
		}
	} catch (error) {
	...
	// Fallback to hardcoded playlist data for the specific FluxGit playlist
	return getFluxGitPlaylistFallback(playlistId);
}

/**
 * Fallback data with actual FluxGit video information
 * This can be updated manually when new videos are added to the playlist
 */
function getFluxGitPlaylistFallback(playlistId: string): YouTubePlaylist {
	const videos: YouTubeVideo[] = [
		{
			id: "NOYK7LTFvZM",
			title: "Using Cursor Hooks for automatic version control",
			description:
				"Here we demonstrate how to use FluxGit with the new Cursor Hooks functionality to automate creating branches for chat sessions and committing work as you go with smart commit messages. Never lose a step again.",
			thumbnail: "https://img.youtube.com/vi/NOYK7LTFvZM/maxresdefault.jpg",
			publishedAt: "2025-09-29T20:50:53+00:00",
			channelTitle: "FluxGit",
			videoId: "NOYK7LTFvZM",
		...
		},
		{
			...
			description:
				"In this episode we'll be showing you how to take multiple Git commits and magically turn them into one. We will do this with reset, rebase and FluxGit (the easy way).",
		...
		},
		{
			...
			description:
				"In this episode, we'll be showing you how to split a commit in Git in a few different ways. With reset, with rebase and with FluxGit (the easy way).",
		...
		},
		{
			...
			description:
				'Here we go over how to "fix up" a commit in both Git and FluxGit. We\'ll look at how to drag and drop a modified file on a commit to amend it in FluxGit and then do the same functional thing using vanilla Git via fixup commits and autosquashing rebase commands.',
		...
		{
			id: "iJ9qJ-xcQ-U",
			title: "Uncommitting in Git and FluxGit",
			description:
				"Scott talks about how to uncommit a commit, something that is not always as simple as you may think, especially uncommitting something in the middle of a series. \n\nHe demonstrates how to do it with two clicks in FluxGit and then how to accomplish the same thing in vanilla Git - once just dropping the commit and then how to do it and still keep the changes in that commit in your working directory.\n\n0:22 - Uncommitting in FluxGit\n1:16 - Uncommitting with the Git CLI\n2:12 - Dropping a commit in Git\n2:58 - Again, but keep the commit’s changes\n4:27 - Wrap up",
